...
| `BROWSER_REQUEST_TIMEOUT_MS` | browser-worker | Optional (`45000`) | Runtime | Public | `45000` | Deploy config | Pages that hang | Ceiling for the whole request. The per-step limits summed still let a pathological page hold a slot |
| `MONITORING_LEASE_MS` | backend | Optional (`120000`) | Runtime | Public | `120000` | Deploy config | Long collections | How long one process holds a source. It **expires** instead of being returned, so a dead process does not freeze the source forever |
| `MONITORING_HISTORY_DAYS` | backend | Optional (`30`) | Runtime | Public | `30` | Deploy config | Retention policy | TTL of the operational log (`monitoring_events`). It stores *what happened*, never payload |
