...
async function stub(
  ...
  await page.route('**/api/assistant/targets', (r) => r.fulfill({ json: opts.targets ?? { floors: [], agents: [], sectors: [], routines: [] } }))
  await page.route('**/api/assistant/projects/*/links', (r) => {
    ligacoes = r.request().postDataJSON() as Record<string, unknown>
    return r.fulfill({ json: { ...COM_PROPOSTA, status: 'draft' } })
  })
  await page.route('**/api/assistant/projects/*/turn', (r) => {
    rodadasAutomaticas += 1
    const t = opts.turn ?? { json: { ...proj, assistantText: 'Por onde as pessoas falam com você hoje?', question: PERGUNTA } }
    return r.fulfill({ status: t.status ?? 200, json: t.json })
  })
  await page.route('**/api/assistant/projects/*/rollback', (r) =>
  ...
  await page.route('**/api/assistant/projects/*/archive', (r) => r.fulfill({ json: { ...COM_PROPOSTA, status: 'archived' } }))
  await page.route('**/api/providers', (r) => r.fulfill({ json: opts.providers ?? PROVEDORES }))
  await page.route('**/api/assistant/projects/*/generate', (r) =>
    r.fulfill({ json: { ...COM_PROPOSTA, assistantText: 'Montei uma primeira proposta.', question: null } }),
  )
  await page.route('**/api/assistant/projects/*/validate', (r) => r.fulfill({ json: { valid: true, issues: PREVIA.issues } }))
  await page.route('**/api/assistant/projects/*/preview', (r) => r.fulfill({ json: previaCorrente }))
  ...
  await page.route('**/api/assistant/projects/*/blueprint', (r) => {
  ...
  await page.route('**/api/assistant/projects/*/layer', (r) => {
  ...
  await page.route('**/api/assistant/projects/*/brief', (r) => {
  ...
  await page.route('**/api/assistant/projects/*/apply', (r) => {
...
test('a primeira tela não é uma folha em branco: traz exemplos clicáveis', async ({ page }) => {
  ...
  await page.getByTestId('assistant-example').first().click()
  await expect(page.getByTestId('assistant-objective')).toHaveValue(/restaurante/i)
})
...
test('descrever a operação leva para a conversa', async ({ page }) => {
  await stub(page)
  await page.goto('/assistant')
  await page.getByTestId('assistant-objective').fill('Quero automatizar o atendimento do meu restaurante')
  await page.getByTestId('assistant-start').click()
  await expect(page).toHaveURL(new RegExp(`/assistant/${PROJETO_ID}$`))
...
test('um projeto existente aparece na lista com o estado dele', async ({ page }) => {
  ...
  await page.goto('/assistant')
  await expect(page.getByTestId(`assistant-project-${PROJETO_ID}`)).toContainText('Aplicada')
  await expect(page.getByTestId(`assistant-project-${PROJETO_ID}`)).toContainText('4/4 obrigatórios')
})
...
test('dá para apagar uma conversa, e a tela avisa que o que ela criou continua de pé', async ({ page }) => {
  ...
  await page.route('**/api/assistant/projects', (r) => r.fulfill({ json: listaAtual }))

  await page.goto('/assistant')
  await expect(page.getByTestId(`assistant-project-${PROJETO_ID}`)).toBeVisible()

  await page.getByTestId(`assistant-delete-${PROJETO_ID}`).click()
  ...
  listaAtual = []
  await page.getByTestId('assistant-delete-confirm').click()

  await expect.poll(() => apagados).toContain(PROJETO_ID)
  await expect(page.getByTestId(`assistant-project-${PROJETO_ID}`)).toHaveCount(0)
...
test('uma conversa que está sendo aplicada não pode ser apagada', async ({ page }) => {
  ...
  await page.goto('/assistant')
  await expect(page.getByTestId(`assistant-delete-${PROJETO_ID}`)).toBeDisabled()
})

// --- a conversa -------------------------------------------------------------------------------

test('o Assistente pergunta uma coisa por vez, com opções e “não sei ainda”', async ({ page }) => {
  await stub(page, { messages: [{ id: 'm1', role: 'user', content: 'quero automatizar o atendimento', createdAt: NOW }] })
  await page.goto(`/assistant/${PROJETO_ID}`)
  await page.getByTestId('assistant-input').fill('quero automatizar')
  ...
  await expect(page.getByTestId('assistant-choice-web')).toBeVisible()
  await expect(page.getByTestId('assistant-unknown')).toBeVisible()
})

test('responder por uma opção manda o rótulo, sem a pessoa digitar', async ({ page }) => {
  await stub(page)
  await page.goto(`/assistant/${PROJETO_ID}`)
  await page.getByTestId('assistant-input').fill('oi')
  await page.getByTestId('assistant-enviar').click()
  await page.getByTestId('assistant-choice-whatsapp').click()
...
test('“não sei ainda” é uma resposta aceita', async ({ page }) => {
  ...
  await page.getByTestId('assistant-input').fill('oi')
  await page.getByTestId('assistant-enviar').click()
  await page.getByTestId('assistant-unknown').click()
...
test('a credencial colada some da tela e vira um aviso', async ({ page }) => {
  ...
  await page.goto(`/assistant/${PROJETO_ID}`)
  await page.getByTestId('assistant-input').fill('minha chave é ghp_abcdefghijklmnopqrstuvwxyz0123')
  await page.getByTestId('assistant-enviar').click()
  await expect(page.getByTestId('assistant-message-system_notice')).toContainText(/credencial/i)
})
...
test('a proposta mostra o que será criado e o que depende da pessoa', async ({ page }) => {
  ...
  await page.goto(`/assistant/${PROJETO_ID}`)
  await expect(page.getByTestId('assistant-counts')).toContainText('4 a criar')
  await expect(page.getByTestId('assistant-counts')).toContainText('2 dependem de você')
  await expect(page.getByTestId('assistant-item-agent-gerente')).toContainText('Marina')
  await expect(page.getByTestId('assistant-item-knowledge-cardapio')).toContainText('Nada é inventado')
  await expect(page.getByTestId('assistant-item-app-canal')).toContainText('Depende de você')
})
...
test('a ficha do agente diz o que ele entrega, com o quê e o que NÃO faz', async ({ page }) => {
  ...
  const ficha = page.getByTestId('assistant-agent-card-gerente')
  await expect(ficha).toContainText('Coordena')
  await expect(page.getByTestId('assistant-agent-objective-gerente')).toContainText('Distribuir a conversa')
  await expect(page.getByTestId('assistant-agent-limits-gerente')).toContainText('Não responde preço nem prazo')
  await expect(page.getByTestId('assistant-agent-executor-gerente')).toContainText('interpreta e responde')
  await expect(page.getByTestId('assistant-agent-tools-gerente')).toContainText('web_chat')
  await expect(page.getByTestId('assistant-agent-handoff-gerente')).toContainText('passa para uma pessoa')
  // Por que ele é um agente separado — a pergunta que decide se há gente demais.
  await expect(page.getByTestId('assistant-agent-rationale-duvidas')).toContainText('o papel é outro')
  // O problema do agente aparece NO agente, e não só numa lista solta em cima.
  await expect(page.getByTestId('assistant-agent-problems-gerente')).toContainText('1 ponto a resolver')
...
test('o que não foi declarado aparece como não declarado', async ({ page }) => {
  ...
  await expect(page.getByTestId('assistant-agent-limits-duvidas')).toContainText('não declarado')
  await expect(page.getByTestId('assistant-agent-handoff-duvidas')).toContainText('não passa para ninguém')
})
...
test('as três camadas são o mesmo plano em três tamanhos — e trocar é uma revisão', async ({ page }) => {
  ...
  const essencial = page.getByTestId('assistant-layer-essential')
  ...
  await expect(page.getByTestId('assistant-layer-complete')).toContainText('2 agentes')
  await expect(page.getByTestId('assistant-layer-complete')).toHaveAttribute('aria-checked', 'true')

  await essencial.click()
  expect(camadaPedida).toBe('essential')
  // O recorte novo vale para tudo: o agente que saiu some da proposta.
  await expect(page.getByTestId('assistant-layer-essential')).toHaveAttribute('aria-checked', 'true')
  await expect(page.getByTestId('assistant-item-agent-duvidas')).toHaveCount(0)
  await expect(page.getByTestId('assistant-item-agent-gerente')).toBeVisible()
})
...
test('"O que entendi" mostra os fatos de onde a proposta saiu', async ({ page }) => {
  ...
  const brief = page.getByTestId('assistant-brief')
  await expect(page.getByTestId('assistant-brief-goal')).toContainText('atender quem chega pelo site')
  // O trabalho aparece como a frase inteira, na ordem em que ele acontece.
  await expect(page.getByTestId('assistant-brief-job-duvidas')).toContainText('Quando alguém manda mensagem no site')
  await expect(page.getByTestId('assistant-brief-job-duvidas')).toContainText('entrega a resposta para a pessoa')
...
test('corrigir o entendimento refaz a proposta, e dá para desfazer', async ({ page }) => {
  ...
  await page.getByTestId('assistant-brief-job-duvidas').click()
  await page.getByTestId('assistant-brief-field-output').fill('a resposta e o link do cardápio')
  await page.getByTestId('assistant-brief-save').click()

  await expect.poll(() => (briefEnviado?.patch as { jobs?: { output: string }[] })?.jobs?.[0]?.output).toBe('a resposta e o link do cardápio')

  await page.getByTestId('assistant-brief-undo').click()
  await expect.poll(() => briefEnviado?.undo).toBe(true)
  // Desfeito, não há mais o que desfazer — e o botão some em vez de mentir.
  await expect(page.getByTestId('assistant-brief-undo')).toHaveCount(0)
})

test('a leitura obsoleta do modelo é dita como obsoleta, não mostrada como atual', async ({ page }) => {
  await stub(page, { project: COM_PROPOSTA })
  await page.goto(`/assistant/${PROJETO_ID}`)
  await expect(page.getByTestId('assistant-llm-status')).toContainText('revisão anterior')
})

test('7) o porquê de cada item aparece — foi gerado e pago, não se joga fora', async ({ page }) => {
  await stub(page, { project: COM_PROPOSTA })
  await page.goto(`/assistant/${PROJETO_ID}`)
  await expect(page.getByTestId('assistant-rationale-agent-gerente')).toContainText('Recebe a conversa e decide quem responde')
  // O que VAI ACONTECER e o PORQUÊ são coisas diferentes, e aparecem separados.
  await expect(page.getByTestId('assistant-item-agent-gerente')).toContainText('Agente novo.')
  await expect(page.getByTestId('assistant-rationale-floor-atendimento')).toContainText('Onde a operação de atendimento mora')
})

test('6) trocar o nome de um agente é edição na tela, sem pedir nada ao modelo', async ({ page }) => {
  await stub(page, { project: COM_PROPOSTA })
  await page.goto(`/assistant/${PROJETO_ID}`)
  await page.getByTestId('assistant-item-edit-agent-gerente').click()

  // O formulário abre com o que já está na proposta — corrigir não é redigitar.
  const bloco = page.getByTestId('assistant-edit-agent-gerente')
  await expect(bloco.getByTestId('assistant-edit-field-name')).toHaveValue('Marina')
  await expect(bloco.getByTestId('assistant-edit-field-objective')).toHaveValue(/Distribuir a conversa/)

  await bloco.getByTestId('assistant-edit-field-name').fill('Recepcionista')
  await bloco.getByTestId('assistant-edit-save').click()

  await expect(page.getByTestId('assistant-item-agent-gerente')).toContainText('Recepcionista')
...
test('8) depois da correção, a tela diz o que mudou', async ({ page }) => {
  await stub(page, { project: COM_PROPOSTA })
  await page.goto(`/assistant/${PROJETO_ID}`)
  await expect(page.getByTestId('assistant-changes')).toHaveCount(0, { timeout: 5000 })

  await page.getByTestId('assistant-item-edit-agent-duvidas').click()
  await page.getByTestId('assistant-edit-field-name').fill('Atendente do salão')
  await page.getByTestId('assistant-edit-save').click()

  await expect(page.getByTestId('assistant-changes')).toContainText('Atendente do salão')
  await expect(page.getByTestId('assistant-changes')).toContainText('Mudou')
})
...
test('tirar um item da proposta é possível, e a recusa do servidor é dita por inteiro', async ({ page }) => {
  ...
  await page.goto(`/assistant/${PROJETO_ID}`)
  await page.getByTestId('assistant-item-edit-agent-duvidas').click()
  await page.getByTestId('assistant-edit-remove').click()
  // Quem depende do item é a única pista útil; trocar por "não foi possível" a apagaria.
  await expect(page.getByTestId('assistant-edit-error')).toContainText('o setor "Atendimento" depende')
  ...
  await expect(page.getByTestId('assistant-edit-agent-duvidas')).toBeVisible()
  await page.getByRole('button', { name: 'Cancelar' }).click()
  await expect(page.getByTestId('assistant-item-agent-duvidas')).toContainText('Rafael')
})
...
test('proposta já aplicada não se edita na tela', async ({ page }) => {
  ...
  await page.goto(`/assistant/${PROJETO_ID}`)
  await expect(page.getByTestId('assistant-item-agent-gerente')).toBeVisible()
  await expect(page.getByTestId('assistant-item-edit-agent-gerente')).toHaveCount(0)
})
...
test('quatro telas, uma por vez — e a escolhida fica marcada', async ({ page }) => {
  ...
  await expect(page.getByTestId('assistant-tab-proposta')).toHaveAttribute('aria-selected', 'true')
  await expect(page.getByTestId('assistant-proposal')).toBeVisible()
  await expect(page.getByTestId('assistant-flow')).toHaveCount(0)

  for (const [aba, alvo] of [
    ['fluxo', 'assistant-flow'],
    ['escritorio', 'assistant-office-preview'],
    ['checklist', 'assistant-checklist'],
    ['proposta', 'assistant-proposal'],
  ] as const) {
    await page.getByTestId(`assistant-tab-${aba}`).click()
    await expect(page.getByTestId(alvo)).toBeVisible()
    await expect(page.getByTestId(`assistant-tab-${aba}`)).toHaveAttribute('aria-selected', 'true')
    // Uma por vez: as outras não ficam montadas atrás.
    const outras = ['assistant-flow', 'assistant-office-preview', 'assistant-checklist', 'assistant-proposal'].filter((t) => t !== alvo)
...
test('sem proposta ainda, não há abas — a conversa é a tela', async ({ page }) => {
  ...
  await page.goto(`/assistant/${PROJETO_ID}`)
  await expect(page.getByTestId('assistant-conversation')).toBeVisible()
  await expect(page.getByTestId('assistant-tabs')).toHaveCount(0)
})
...
test('o fluxo mostra quem coordena e quem é acionado — não só a lista', async ({ page }) => {
  await stub(page, { project: COM_PROPOSTA })
  await page.goto(`/assistant/${PROJETO_ID}`)
  await page.getByTestId('assistant-tab-fluxo').click()

  const fluxo = page.getByTestId('assistant-flow')
  ...
  await expect(page.getByTestId('assistant-flow-floor-atendimento')).toContainText('Atendimento do Restaurante')
  await expect(page.getByTestId('assistant-flow-sector-setor')).toContainText('Mesa de Atendimento')
  await expect(page.getByTestId('assistant-flow-sector-setor')).toContainText('o coordenador decide quem responde')
  // Quem coordena, quem é acionado, e o papel de cada um.
  await expect(page.getByTestId('assistant-flow-agent-gerente')).toContainText('Marina')
  await expect(page.getByTestId('assistant-flow-agent-gerente')).toContainText('coordena')
  await expect(page.getByTestId('assistant-flow-sector-setor')).toContainText('aciona')
  await expect(page.getByTestId('assistant-flow-agent-duvidas')).toContainText('busca informação')
  // E o que depende da pessoa aparece junto do desenho.
  await expect(page.getByTestId('assistant-flow-needs')).toContainText('web_chat')
})
...
test('o crítico aparece antes da lista, com o conserto ao lado', async ({ page }) => {
  ...
  const critica = page.getByTestId('assistant-critique')
  await expect(critica).toBeVisible()
  // O que trava vem marcado como trava; o que vale ver, como aviso.
  const gerente = page.getByTestId('assistant-finding-manager_without_team')
  ...
  await expect(page.getByTestId('assistant-finding-orphan_agent')).toContainText('Vale ver')

  // A leitura da operação carrega o FATO de cada nota — nota sem fato é palpite com número.
  const score = page.getByTestId('assistant-score')
...
test('o ensaio mostra o caminho e diz que nada foi executado', async ({ page }) => {
  ...
  const ensaio = page.getByTestId('assistant-simulation')
  ...
  await expect(page.getByTestId('assistant-scenario-job:duvida')).toContainText('Marina → Rafael')
  // O cenário que falhou aparece com o motivo e o conserto.
  const aprovacao = page.getByTestId('assistant-scenario-aprovacao')
...
test('o motivo de cada agente existir fica visível', async ({ page }) => {
  await stub(page, { project: COM_PROPOSTA })
  await page.goto(`/assistant/${PROJETO_ID}`)
  const bloco = page.getByTestId('assistant-mergesplit')
...
test('o que preocupa aparece como aviso, e não como erro', async ({ page }) => {
  await stub(page, { project: COM_PROPOSTA })
  await page.goto(`/assistant/${PROJETO_ID}`)
  await expect(page.getByTestId('assistant-warnings')).toContainText('web_chat ainda não está conectado')
  await expect(page.getByTestId('assistant-issues')).toHaveCount(0)
})
...
test('o que bloqueia aparece com o que fazer, e o botão de aplicar fica travado', async ({ page }) => {
  ...
  await page.goto(`/assistant/${PROJETO_ID}`)
  await expect(page.getByTestId('assistant-issues')).toContainText('precisa de pelo menos um especialista')
  await expect(page.getByTestId('assistant-issues')).toContainText('inclua outro agente')
  await expect(page.getByTestId('assistant-apply')).toBeDisabled()
})

test('as suposições ficam visíveis, e não escondidas na proposta', async ({ page }) => {
  await stub(page, { project: COM_PROPOSTA })
  await page.goto(`/assistant/${PROJETO_ID}`)
  await expect(page.getByTestId('assistant-assumptions')).toContainText('horário comercial')
})

test('o JSON existe, mas fica em “Avançado”', async ({ page }) => {
  await stub(page, { project: COM_PROPOSTA })
  await page.goto(`/assistant/${PROJETO_ID}`)
  await expect(page.getByTestId('assistant-proposal')).not.toContainText('blueprintPatch')
  await page.getByTestId('assistant-advanced').getByText('Avançado').click()
  await expect(page.getByTestId('assistant-advanced')).toContainText('knowledgeRequirements')
})
...
test('1) o reaproveitamento proposto já vem apontado para o recurso de mesmo nome', async ({ page }) => {
  ...
  await expect(page.getByTestId('assistant-link-suggested-floor-atendimento')).toContainText('Atendimento do Restaurante')
  await expect(page.getByTestId('assistant-link-floor-atendimento')).toHaveValue('reuse|f9')

  await page.getByTestId('assistant-links-save').click()
...
test('sem um nome igual, nada é adivinhado', async ({ page }) => {
  ...
  await page.goto(`/assistant/${PROJETO_ID}`)
  await expect(page.getByTestId('assistant-link-suggested-floor-atendimento')).toHaveCount(0)
  await expect(page.getByTestId('assistant-link-floor-atendimento')).toHaveValue('')
})
...
test('3) dá para colar o conteúdo do conhecimento na própria proposta', async ({ page }) => {
  await stub(page, { project: COM_PROPOSTA })
  await page.goto(`/assistant/${PROJETO_ID}`)
  await page.getByTestId('assistant-item-edit-knowledge-cardapio').click()

  await expect(page.getByText(/nada é inventado/i)).toBeVisible()
  await page.getByTestId('assistant-edit-field-content').fill('Pizza margherita 40. Refrigerante 8.')
  await page.getByTestId('assistant-edit-save').click()
...
test('4) a espera é anunciada, e não parece uma tela travada', async ({ page }) => {
  ...
  await page.route('**/api/assistant/projects/*/messages', async (r) => {
  ...
  const espera = page.getByTestId('assistant-thinking')
...
test('5) a falha já resolvida sai do alarme, mas fica no histórico', async ({ page }) => {
  ...
  // ler e decidir. Abrir é o primeiro passo de todo caso que é sobre a conversa.
  await page.getByTestId('assistant-launcher').click()
  const aviso = page.getByTestId('assistant-message-system_notice')
...
test('no desktop o chat FLUTUA: ele não toma largura de nada', async ({ page }) => {
  ...
  const comChat = (await page.getByTestId('assistant-workspace').boundingBox())!.width
  await page.getByTestId('assistant-close').click()
  await expect(painel).toBeHidden()
  const semChat = (await page.getByTestId('assistant-workspace').boundingBox())!.width
...
test('fechar e reabrir preserva a conversa e o que estava digitado', async ({ page }) => {
  ...
  // ler e decidir. Abrir é o primeiro passo de todo caso que é sobre a conversa.
  await page.getByTestId('assistant-launcher').click()
  await expect(page.getByTestId('assistant-message-assistant')).toContainText('Montei a proposta.')

  await page.getByTestId('assistant-input').fill('quero trocar o nome da Marina')
  await page.getByTestId('assistant-close').click()
  await page.getByTestId('assistant-launcher').click()
  ...
  await expect(page.getByTestId('assistant-input')).toHaveValue('quero trocar o nome da Marina')
  await expect(page.getByTestId('assistant-message-assistant')).toContainText('Montei a proposta.')
})
...
test('trocar de tela não mexe na conversa — e existe UMA conversa só', async ({ page }) => {
  ...
  await page.getByTestId('assistant-input').fill('rascunho de pergunta')
  await page.getByTestId('assistant-tab-escritorio').click()
  await page.getByTestId('assistant-tab-checklist').click()
  await expect(page.getByTestId('assistant-input')).toHaveValue('rascunho de pergunta')
...
test('o escritório mostra os agentes e setores da proposta', async ({ page }) => {
  ...
  await expect(page.getByTestId('assistant-office-totals')).toContainText('2 agentes')
  await expect(page.getByTestId('assistant-office-totals')).toContainText('1 setor')
  await expect(page.getByTestId('assistant-office-totals')).toContainText('1 andar')

  // O mesmo mapa da página inicial, com a descrição em texto ao lado dele.
  await expect(page.getByTestId('assistant-office-map')).toBeVisible()
  const descricao = page.getByTestId('assistant-office-description')
  ...
  await expect(page.getByTestId('assistant-office-notice')).toContainText('nada aqui existe ainda')
})
...
test('a prévia acompanha a proposta: editar um nome redesenha o escritório', async ({ page }) => {
  ...
  await page.getByTestId('assistant-item-edit-agent-gerente').click()
  await page.getByTestId('assistant-edit-field-name').fill('Bruna')
  await page.getByTestId('assistant-edit-save').click()

  await page.getByTestId('assistant-tab-escritorio').click()
  // Sem recarregar nada: o desenho vem do blueprint que acabou de mudar.
  await expect(page.getByTestId('assistant-office-description')).toContainText('Bruna')
})
...
test('agente fora de setor aparece na área comum', async ({ page }) => {
  ...
  await expect(page.getByTestId('assistant-office-description')).toContainText('área comum')
  await expect(page.getByTestId('assistant-office-description')).toContainText('Tereza')
})
...
test('com vários andares, dá para escolher qual olhar', async ({ page }) => {
  ...
  await expect(page.getByTestId('assistant-office-totals')).toContainText('2 andares')
  await page.getByTestId('assistant-office-floor-select').selectOption('mesa-analise')
  await expect(page.getByTestId('assistant-office-description')).toContainText('Mesa de Análise')
  await expect(page.getByTestId('assistant-office-description')).toContainText('Bruno')
})
...
test('no rascunho, os agentes não são botões nem param o teclado', async ({ page }) => {
  ...
  await page.goto(`/assistant/${PROJETO_ID}`)
  await page.getByTestId('assistant-tab-escritorio').click()
  const mapa = page.getByTestId('assistant-office-map')
  ...
  await page.getByTestId('assistant-tab-escritorio').focus()
  for (let i = 0; i < 15; i++) {
  ...
  expect(page.url()).toContain(`/assistant/${PROJETO_ID}`)
})
...
test('uma revisão não arrasta a pessoa para outra tela', async ({ page }) => {
  ...
  await page.getByTestId('assistant-input').fill('troque o nome da Marina')
  await page.getByTestId('assistant-enviar').click()
  ...
  await expect(page.getByTestId('assistant-office-map')).toBeVisible()
  await expect(page.getByTestId('assistant-tab-escritorio')).toHaveAttribute('aria-selected', 'true')
})
...
test('só provedor configurado é oferecido', async ({ page }) => {
  ...
  await page.goto(`/assistant/${PROJETO_ID}`)
  await page.getByTestId('assistant-advanced').getByText('Avançado').click()
  const seletor = page.getByTestId('assistant-provider-select')
...
test('o seletor de modelo mostra o RÓTULO e envia o id', async ({ page }) => {
  ...
  const modelo = page.getByTestId('assistant-model-select')
...
test('sem provedor nenhum configurado, a tela manda para Configurações', async ({ page }) => {
  ...
  await page.goto(`/assistant/${PROJETO_ID}`)
  await page.getByTestId('assistant-advanced').getByText('Avançado').click()
  await expect(page.getByTestId('assistant-provider')).toContainText('Nenhum provedor configurado')
  await expect(page.getByTestId('assistant-provider-select')).toHaveCount(0)
})
...
test('a escolha de recurso só oferece o que é desta conta, e manda ação e id', async ({ page }) => {
  ...
  await page.goto(`/assistant/${PROJETO_ID}`)
  const seletor = page.getByTestId('assistant-link-floor-atendimento')
  await expect(seletor).toBeVisible()
...
test('sem nada para reaproveitar, o bloco não existe — e com algo, ele aparece', async ({ page }) => {
  ...
  await page.goto(`/assistant/${PROJETO_ID}`)
  await expect(page.getByTestId('assistant-proposal')).toBeVisible()
  await expect(page.getByTestId('assistant-links-editor')).toHaveCount(0)
  await expect(page.getByTestId('assistant-no-targets')).toHaveCount(0)
  ...
  await page.goto(`/assistant/${PROJETO_ID}`)
  await expect(page.getByTestId('assistant-links-editor')).toBeVisible()
})
...
test('a proposta travada por reaproveitamento tem saída na tela', async ({ page }) => {
  ...
  const bloco = page.getByTestId('assistant-links-pending')
  await expect(bloco).toContainText('travando a aplicação')
  await page.getByTestId('assistant-links-create-pending').click()
...
test('o que foi marcado na confirmação é o que vai no pedido', async ({ page }) => {
  ...
  await page.getByTestId('assistant-approve-apps').getByText('web_chat').click()
  await page.getByTestId('assistant-apply-confirm').click()
...
test('um projeto travado em “aplicando” oferece retomar', async ({ page }) => {
  ...
  await page.goto(`/assistant/${PROJETO_ID}`)
  await expect(page.getByTestId('assistant-applying')).toBeVisible()
  await page.getByTestId('assistant-resume').click()
  await expect(page.getByTestId('assistant-status').first()).toContainText('Aplicada')
})
...
test('a falha mostra o motivo e quantos recursos ficaram de pé', async ({ page }) => {
  ...
  await page.goto(`/assistant/${PROJETO_ID}`)
  await expect(page.getByTestId('assistant-failed')).toContainText('continua de pé')
  await expect(page.getByTestId('assistant-failure-reason')).toContainText('o andar do agente não foi criado')
})
...
test('depois de aplicar, os passos ficam visíveis em “Avançado”', async ({ page }) => {
  ...
  await page.getByTestId('assistant-apply').click()
  await page.getByTestId('assistant-apply-confirm').click()
  await page.getByTestId('assistant-tab-proposta').click()
  await page.getByTestId('assistant-advanced').getByText('Avançado').click()
  await expect(page.getByTestId('assistant-steps')).toContainText('floor: atendimento')
  await expect(page.getByTestId('assistant-steps')).toContainText('não está conectado')
})
...
test('recarregar um projeto aplicado reconstrói os links', async ({ page }) => {
  ...
  await page.goto(`/assistant/${PROJETO_ID}`)
  await expect(page.getByTestId('assistant-links')).toBeVisible()
  await expect(page.getByTestId('assistant-link-agent')).toHaveAttribute('href', '/agents/a1')
})
...
test('arquivar avisa que nada é removido, e exige confirmar', async ({ page }) => {
  ...
  await page.goto(`/assistant/${PROJETO_ID}`)
  await page.getByTestId('assistant-advanced').getByText('Avançado').click()
  await page.getByTestId('assistant-archive').click()
  await expect(page.getByTestId('assistant-confirm-archive')).toContainText('Nada do que ele criou é removido')
  await page.getByTestId('assistant-archive-confirm').click()
  await expect(page.getByTestId('assistant-status').first()).toContainText('Arquivada')
})
...
test('desfazer diz o impacto antes, e o que sobrou depois', async ({ page }) => {
  ...
  await page.goto(`/assistant/${PROJETO_ID}`)
  await page.getByTestId('assistant-advanced').getByText('Avançado').click()
  await page.getByTestId('assistant-rollback').click()
  await expect(page.getByTestId('assistant-confirm-rollback')).toContainText('o que já existia e o que foi criado por outra')

  await page.getByTestId('assistant-rollback-confirm').click()
  await expect(page.getByTestId('assistant-rollback-result')).toContainText('2 removidos')
  await expect(page.getByTestId('assistant-rollback-result')).toContainText('único andar do prédio')
})
...
test('aplicar exige revisar antes, e manda o hash da proposta revisada', async ({ page }) => {
  ...
  const dialogo = page.getByTestId('assistant-apply-dialog')
...
test('dar acesso a um App é uma decisão à parte, e vem desmarcada', async ({ page }) => {
  ...
  await page.goto(`/assistant/${PROJETO_ID}`)
  await page.getByTestId('assistant-apply').click()
  await expect(page.getByTestId('assistant-approve-apps')).toContainText('Sem marcar, os agentes são criados sem acesso')
...
test('uma mudança em algo que já existe exige aprovação individual', async ({ page }) => {
  ...
  await page.goto(`/assistant/${PROJETO_ID}`)
  await page.getByTestId('assistant-apply').click()
  await expect(page.getByTestId('assistant-approve-updates')).toContainText('Agente que já existia')
  // Desmarcada, a confirmação não sai.
  await expect(page.getByTestId('assistant-apply-confirm')).toBeDisabled()
  await page.getByText('Alterar Agente: Agente que já existia').click()
  await expect(page.getByTestId('assistant-apply-confirm')).toBeEnabled()
})
...
test('uma confirmação sobre prévia velha é recusada com o motivo', async ({ page }) => {
  ...
  await page.getByTestId('assistant-apply').click()
  await page.getByTestId('assistant-apply-confirm').click()
  await expect(page.getByTestId('assistant-apply-error')).toContainText('revise de novo')
})

// --- falhas que a pessoa precisa entender ----------------------------------------------------------------------

test('sem chave de provedor, a tela manda para Configurações', async ({ page }) => {
  await stub(page, { turn: { status: 400, json: { code: 'no_provider_key', message: 'Configure a chave do provedor em Configurações para o Assistente poder trabalhar.' } } })
  await page.goto(`/assistant/${PROJETO_ID}`)
  await page.getByTestId('assistant-input').fill('oi')
  await page.getByTestId('assistant-enviar').click()
  await expect(page.getByTestId('assistant-error')).toContainText('Configure a chave do provedor')
  await expect(page.getByTestId('assistant-settings-link')).toHaveAttribute('href', '/settings')
})
...
test('limite de tokens atingido é dito com todas as letras', async ({ page }) => {
  ...
  await page.getByTestId('assistant-input').fill('oi')
  await page.getByTestId('assistant-enviar').click()
  await expect(page.getByTestId('assistant-error')).toContainText('limite mensal de tokens')
})
...
test('obrigatório e opcional são contados separados, e “pronto” não mente', async ({ page }) => {
  ...
  await page.goto(`/assistant/${PROJETO_ID}`)
  await page.getByTestId('assistant-tab-checklist').click()
  await expect(page.getByTestId('assistant-required-progress')).toHaveText('0/4')
  await expect(page.getByTestId('assistant-ready')).toContainText('Ainda faltam pendências obrigatórias')
})
...
test('o item automático não tem botão de marcar; o manual tem', async ({ page }) => {
  ...
  await page.goto(`/assistant/${PROJETO_ID}`)
  await page.getByTestId('assistant-tab-checklist').click()
  await expect(page.getByTestId('assistant-check-auto-app:canal')).toContainText('conferido pelo sistema')
  await expect(page.getByTestId('assistant-check-toggle-app:canal')).toHaveCount(0)
  await expect(page.getByTestId('assistant-check-toggle-test:conversa-de-teste')).toBeVisible()
})
...
test('a pendência leva direto ao lugar de resolver', async ({ page }) => {
  ...
  await page.goto(`/assistant/${PROJETO_ID}`)
  await page.getByTestId('assistant-tab-checklist').click()
  await expect(page.getByTestId('assistant-check-link-app:canal')).toHaveAttribute('href', '/apps')
})
...
test('2) a pendência de conhecimento leva ao agente que vai receber o documento', async ({ page }) => {
  ...
  await expect(page.getByTestId('assistant-check-link-knowledge:cardapio')).toHaveAttribute('href', '/agents/a1')
})
...
test('reconferir apura de novo contra o estado real', async ({ page }) => {
  ...
  await expect(page.getByTestId('assistant-check-app:canal')).toContainText('Conectar web_chat')
  await expect(page.getByTestId('assistant-check-auto-app:canal')).toBeVisible()
})
 ...
 * "Assistente", "Blueprint" e "Montar operação" viravam três produtos que a pessoa precisava
 * descobrir sozinha que eram a mesma coisa.
 *
 * Ela é um MODO DE TRABALHO do Assistente. A porta é o botão dentro do chat, que é onde a
 * conversa já está acontecendo — e a rota `/assistant` continua inteira, para deep link,
...
test('no celular a conversa é uma TELA, e sair dela devolve a proposta', async ({ page }) => {
   ...
   * ela deixou de ser uma caixa desta página: é o painel do Assistente, o mesmo de todas as
  ...
  await page.getByTestId('assistant-tab-proposta').click()
  await expect(page.getByTestId('assistant-counts')).toBeVisible()
...
test('em 320 px nada estoura para os lados — nas quatro telas', async ({ page }) => {
  ...
  for (const rota of ['/assistant', `/assistant/${PROJETO_ID}`]) {
    await page.goto(rota)
    if (rota !== '/assistant') await page.getByTestId('assistant-launcher').click()
    await expect(page.getByTestId(rota === '/assistant' ? 'assistant-projects' : 'assistant-conversation')).toBeVisible()
    ...
    if (rota !== '/assistant') await page.getByTestId('assistant-close').click()
  }
...
test('a lista do que entra no ar nasce VAZIA, e o pedido vai sem ativação nenhuma', async ({ page }) => {
  ...
  const secao = page.getByTestId('assistant-approve-activation')
...
test('marcar uma fonte manda a key dela — e só ela', async ({ page }) => {
  ...
  await page.getByTestId('assistant-approve-activation').getByText('Fonte: Cotações CXSE3').click()
  await page.getByTestId('assistant-apply-confirm').click()
...
test('sem nada ativável, a seção não existe — nenhuma pergunta sem resposta possível', async ({ page }) => {
  ...
  await page.goto(`/assistant/${PROJETO_ID}`)
  await page.getByTestId('assistant-apply').click()
  await expect(page.getByTestId('assistant-apply-dialog')).toBeVisible()
  await expect(page.getByTestId('assistant-approve-activation')).toHaveCount(0)
})
...
test('a proposta mostra o que o V2 acrescenta, agrupado e com nome de produto', async ({ page }) => {
  ...
  const proposta = page.getByTestId('assistant-proposal')
...
test('a entrega pede uma CONEXÃO na hora de aplicar, e o padrão é não escolher', async ({ page }) => {
  ...
  const secao = page.getByTestId('assistant-delivery-connection')
  await expect(secao).toBeVisible()
  ...
  // não pediu.
  await expect(page.getByTestId('assistant-delivery-entrega')).toHaveValue('')
...
test('escolher a conexão manda a referência — nunca o endereço', async ({ page }) => {
  ...
  await page.goto(`/assistant/${PROJETO_ID}`)
  await page.getByTestId('assistant-apply').click()
  await page.getByTestId('assistant-delivery-entrega').selectOption('c1')
  await page.getByTestId('assistant-apply-confirm').click()
...
test('sem conexão nenhuma, a tela diz o que fazer em vez de oferecer um vazio mudo', async ({ page }) => {
  ...
  await page.goto(`/assistant/${PROJETO_ID}`)
  await page.getByTestId('assistant-apply').click()
  await expect(page.getByTestId('assistant-delivery-connection')).toContainText('ainda não tem uma conexão de envio')
})
