...
describe('planDesks', () => {
  ...
  it('uses a single desk for the exact sizes and combines for the rest', () => {
    ...
    expect(planDesks(2, rng).map((d) => d.art)).toEqual(['mesa-2-3x2'])
    ...
    expect(planDesks(3, rng).map((d) => d.art)).toEqual(['mesa-2-3x2', 'mesa-1-frente-2x2'])
    expect(planDesks(9, rng).map((d) => d.art)).toEqual(['mesa-6-4.5x3', 'mesa-2-3x2', 'mesa-1-frente-2x2'])
  })

  it('faces a lone agent front or back by chance', () => {
