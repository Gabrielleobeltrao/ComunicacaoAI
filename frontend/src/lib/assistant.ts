...
export type AssistantStatus = 'discovery' | 'draft' | 'ready' | 'applying' | 'applied' | 'failed' | 'archived'
...
export interface AssistantProject {
  ...
  /** O que o Assistente entendeu do negócio. É o que a tela mostra como "O que entendi". */
...
export interface TurnResponse extends AssistantProject {
  assistantText: string
  question: AssistantQuestion | null
  secretMasked?: boolean
}

export interface ApplyResponse extends AssistantProject {
...
const request = async <T>(path: string, init?: RequestInit): Promise<T> => {
  const res = await fetch(`${API_URL}/api/assistant${path}`, {
    credentials: 'include',
  ...
  if (!res.ok) {
    ...
    throw new AssistantError(body?.code ?? 'error', body?.message ?? body?.error ?? 'Não foi possível concluir.')
  }
  return res.status === 204 ? (null as T) : ((await res.json()) as T)
}

export const listProjects = (includeArchived = false) => request<AssistantProject[]>(`/projects${includeArchived ? '?includeArchived=true' : ''}`)
export const createProject = (objective: string) => request<AssistantProject>('/projects', { method: 'POST', body: JSON.stringify({ objective }) })
export const getProject = (id: string) => request<AssistantProject>(`/projects/${id}`)
export const listMessages = (id: string) => request<AssistantMessage[]>(`/projects/${id}/messages`)
...
export const previewProject = (id: string) => request<AssistantPreview>(`/projects/${id}/preview`)
export const listTargets = () => request<AssistantTargets>('/targets')
export const setLinks = (id: string, links: BlueprintLink[]) => request<AssistantProject>(`/projects/${id}/links`, { method: 'PATCH', body: JSON.stringify({ links }) })
...
export const setLayer = (id: string, layer: BlueprintLayer) =>
  request<AssistantProject>(`/projects/${id}/layer`, { method: 'PATCH', body: JSON.stringify({ layer }) })

export const editBrief = (id: string, patch: Partial<OperationBrief>) =>
  request<AssistantProject>(`/projects/${id}/brief`, { method: 'PATCH', body: JSON.stringify({ patch }) })

export const undoBrief = (id: string) => request<AssistantProject>(`/projects/${id}/brief`, { method: 'PATCH', body: JSON.stringify({ undo: true }) })

export const editBlueprint = (id: string, edits: BlueprintEdit[]) =>
  request<AssistantProject>(`/projects/${id}/blueprint`, { method: 'PATCH', body: JSON.stringify({ edits }) })
export const rollbackProject = (id: string) => request<AssistantProject & { removed: string[]; kept: { key: string; reason: string }[] }>(`/projects/${id}/rollback`, { method: 'POST' })
/**
 ...
 */
export type AssistantProvider = ProviderInfo & { configured: boolean }

export const listProviders = () =>
  fetch(`${API_URL}/api/providers`, { credentials: 'include' })
    .then((r) => (r.ok ? r.json() : []))
    .then((v) => v as AssistantProvider[])
export const patchProject = (id: string, patch: { provider?: 'anthropic' | 'openai'; model?: string | null; title?: string }) =>
  request<AssistantProject>(`/projects/${id}`, { method: 'PATCH', body: JSON.stringify(patch) })
...
export const markChecklistItem = (id: string, itemId: string, done: boolean) =>
  request<AssistantProject>(`/projects/${id}/checklist/${encodeURIComponent(itemId)}`, { method: 'PATCH', body: JSON.stringify({ done }) })
export const archiveProject = (id: string) => request<AssistantProject>(`/projects/${id}/archive`, { method: 'POST' })
