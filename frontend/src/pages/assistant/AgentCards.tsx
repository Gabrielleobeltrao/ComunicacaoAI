...
import type { AssistantPreview, Blueprint } from '../../lib/assistant'
...
export function AgentFicha({ agentKey, blueprint, preview }: { agentKey: string; blueprint: Blueprint | null | undefined; preview: AssistantPreview | null }) {
  ...
  return (
    <div className="flex flex-col gap-2" style={{ marginTop: 6 }} data-testid={`assistant-agent-card-${agentKey}`}>
      <div className="flex flex-wrap items-center gap-2">
        ...
        {problemas.length > 0 && (
          <Badge tone={problemas.some((p) => p.severity === 'error') ? 'danger' : 'warning'} data-testid={`assistant-agent-problems-${agentKey}`}>
      ...
      <div className="grid gap-2" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(190px, 1fr))' }}>
        <Campo label="Entrega" testid={`assistant-agent-objective-${agentKey}`}>{agente.objective || naoDeclarado}</Campo>
        <Campo label="Acionado quando">{agente.role || naoDeclarado}</Campo>
        <Campo label="O que NÃO faz" testid={`assistant-agent-limits-${agentKey}`}>{agente.constraints || naoDeclarado}</Campo>
        <Campo label="Como executa" testid={`assistant-agent-executor-${agentKey}`}>
        ...
        <Campo label="Ferramentas" testid={`assistant-agent-tools-${agentKey}`}>{ferramentas.length > 0 ? ferramentas.map((f) => f.appKey).join(', ') : 'nenhuma'}</Campo>
        <Campo label="Acionamento" testid={`assistant-agent-activation-${agentKey}`}>
          {[
        ...
        <Campo label="Sai da mão dele" testid={`assistant-agent-handoff-${agentKey}`}>
      ...
      {(porque || agente.rationale) && (
        <Campo label="Por que separado" testid={`assistant-agent-rationale-${agentKey}`}>{porque?.rationale || agente.rationale}</Campo>
      )}

      {agente.layerReason && (
        <span style={{ fontSize: 12, color: 'var(--text-faint)' }} data-testid={`assistant-agent-layer-${agentKey}`}>
