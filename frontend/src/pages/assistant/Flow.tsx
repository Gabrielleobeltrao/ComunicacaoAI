...
export function Flow({ blueprint }: { blueprint: Blueprint | null | undefined }) {
  ...
  const Pessoa = ({ chave, coordenador }: { chave: string; coordenador?: boolean }) => {
    ...
    if (!a) return null
    return (
      <div className="flex flex-wrap items-baseline gap-x-2" data-testid={`assistant-flow-agent-${chave}`}>
  ...
  return (
    <Card>
      <div className="flex flex-col gap-3" data-testid="assistant-flow">
        <div>
        ...
        {(blueprint.floors ?? []).map((andar) => {
          ...
          return (
            <div key={andar.key} className="flex flex-col gap-2" data-testid={`assistant-flow-floor-${andar.key}`}>
              ...
              {setoresDoAndar.map((setor) => (
                <div
                  ...
                  data-testid={`assistant-flow-sector-${setor.key}`}
                >
                  <div className="flex flex-wrap items-center gap-2">
        ...
        {rotinas.length > 0 && (
          <div className="flex flex-col gap-1" data-testid="assistant-flow-routines">
        ...
        {(blueprint.appRequirements ?? []).length + (blueprint.knowledgeRequirements ?? []).length > 0 && (
          <div className="flex flex-col gap-1" data-testid="assistant-flow-needs">
