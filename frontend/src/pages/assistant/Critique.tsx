...
import type { AssistantPreview, ArchitectureScore } from '../../lib/assistant'
...
export function Critique({ preview }: { preview: AssistantPreview | null }) {
  const critica = preview?.critique
  const ensaio = preview?.simulation
  if (!critica && !ensaio) return null

  return (
    <div className="flex flex-col gap-3" data-testid="assistant-critique">
        ...
        <Card>
          <div className="flex flex-col gap-2">
            ...
            {critica.llmStatus && critica.llmStatus !== 'ok' && (
              <p style={{ fontSize: 12, color: 'var(--text-faint)' }} data-testid="assistant-llm-status">
            ...
            {critica.findings.map((f, i) => (
              <div key={`${f.code}-${i}`} className="flex flex-wrap items-start gap-2" data-testid={`assistant-finding-${f.code}`}>
      ...
      {critica && (
        <Card tone="sunken">
          <div className="flex flex-col gap-2" data-testid="assistant-score">
            <div>
      ...
      {ensaio && ensaio.results.length > 0 && (
        <Card>
          <div className="flex flex-col gap-2" data-testid="assistant-simulation">
            <div>
            ...
            {ensaio.results.map((r) => {
              ...
              return (
                <div key={r.caseId} className="flex flex-col gap-1" data-testid={`assistant-scenario-${r.caseId}`}>
      ...
      {critica && critica.mergeSplit.length > 0 && (
        <Card tone="sunken">
          <div className="flex flex-col gap-1" data-testid="assistant-mergesplit">
