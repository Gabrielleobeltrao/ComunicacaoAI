...
import type { AssistantProject, AssistantTargets, BlueprintLink } from '../../lib/assistant'
...
const VAZIO: AssistantTargets = { floors: [], agents: [], sectors: [], routines: [], databases: [], sources: [], monitors: [], flows: [], connections: [] }
...
export function ResourceLinks({ project, onSalvar, carregando }: { project: AssistantProject; onSalvar: (links: BlueprintLink[]) => void; carregando: boolean }) {
  const [alvos, setAlvos] = useState<AssistantTargets>(VAZIO)
  ...
  return (
    <Card>
      <div className="flex flex-col gap-2" data-testid="assistant-links-editor">
        <strong style={{ fontSize: 13 }}>Aproveitar o que já existe</strong>
        {pendentes.length > 0 && (
          <div className="flex flex-col gap-2" data-testid="assistant-links-pending">
            ...
            <div>
              <Button variant="secondary" onClick={criarOsPendentes} disabled={carregando} data-testid="assistant-links-create-pending">
        ...
        {total === 0 ? (
          <p style={{ fontSize: 12.5, color: 'var(--text-muted)' }} data-testid="assistant-no-targets">
          ...
          <>
            {itens.map((item) => {
              ...
              <label key={`${item.kind}:${item.key}`} className="flex flex-col gap-1" style={{ fontSize: 12.5 }}>
                ...
                {sugerido && (
                  <span style={{ color: 'var(--text-muted)' }} data-testid={`assistant-link-suggested-${item.kind}-${item.key}`}>
                    A proposta pediu para reaproveitar — encontrei “{sugerido.name}” com este nome. Confirme abaixo.
                  </span>
                )}
                <Select
                  data-testid={`assistant-link-${item.kind}-${item.key}`}
                  value={valorDe(item)}
            ...
            <div>
              <Button variant="secondary" onClick={salvar} disabled={carregando} data-testid="assistant-links-save">
