...
import type { AssistantProject, OperationBrief } from '../../lib/assistant'
...
export function Brief({
  ...
  return (
    <Card>
      <div className="flex flex-col gap-3" data-testid="assistant-brief">
        <div className="flex flex-wrap items-start justify-between gap-2">
          ...
          {editavel && project.canUndoBrief && (
            <Button variant="secondary" onClick={onDesfazer} disabled={carregando || salvando} data-testid="assistant-brief-undo">
        ...
        {brief.businessGoal && (
          <div>
            ...
            <p style={{ fontSize: 13 }} data-testid="assistant-brief-goal">
              {brief.businessGoal}
            </p>
          </div>
        )}
          ...
          <div className="flex flex-col gap-2">
            ...
            {brief.jobs.map((j) =>
              editando === j.id && rascunho ? (
                <div key={j.id} className="flex flex-col gap-2" style={{ padding: 10, borderRadius: 10, background: 'var(--surface-sunken)' }} data-testid={`assistant-brief-edit-${j.id}`}>
                  {CAMPOS.map((campo) => (
                    <label key={campo.nome} className="flex flex-col gap-1" style={{ fontSize: 12, color: 'var(--text-muted)' }}>
                      ...
                      {campo.nome === 'name' ? (
                        <Input
                          ...
                          data-testid={`assistant-brief-field-${campo.nome}`}
                        />
                      ) : (
                        <Textarea
                          rows={2}
                  ...
                  {erro && (
                    <p style={{ fontSize: 12.5, color: 'var(--intent-danger-text)' }} data-testid="assistant-brief-error">
                      {erro}
                    </p>
                  )}
                  <div className="flex flex-wrap gap-2">
                    <Button onClick={salvarTrabalho} disabled={salvando} data-testid="assistant-brief-save">
                ...
                <button
                  ...
                  data-testid={`assistant-brief-job-${j.id}`}
