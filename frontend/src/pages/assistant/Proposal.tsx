...
import type { AssistantPreview, AssistantProject, Blueprint, BlueprintEdit, BlueprintLayer, PreviewItem } from '../../lib/assistant'
...
export function Proposal({
  ...
  if (!project.hasBlueprint) {
    return (
      <Card>
        <p style={{ fontSize: 13, color: 'var(--text-muted)' }} data-testid="assistant-no-proposal">
          A proposta aparece aqui quando o Assistente tiver o suficiente. Responda as perguntas ao lado, ou peça uma primeira proposta agora.
        </p>
      </Card>
    )
  }
  ...
  return (
    <div className="flex flex-col gap-3" data-testid="assistant-proposal">
      ...
      {preview && (
        ...
        >
          <div className="flex flex-wrap items-center gap-2" data-testid="assistant-counts">
          ...
          <div className="flex flex-wrap gap-2">
            <Button variant="secondary" onClick={onRevisar} disabled={carregando} data-testid="assistant-review">
              Revisar mudanças
            </Button>
            <Button onClick={onAplicar} disabled={carregando || !preview?.valid} data-testid="assistant-apply">
              Aplicar
            </Button>
          </div>
        </div>
      ...
      {erros.length > 0 && (
        <Card>
          <div className="flex flex-col gap-2" data-testid="assistant-issues">
      ...
      {mudancas.length > 0 && (
        <Card tone="sunken">
          <div className="flex flex-col gap-2" data-testid="assistant-changes">
            <div>
      ...
      {avisos.length > 0 && (
        <Card tone="sunken">
          <div className="flex flex-col gap-1" data-testid="assistant-warnings">
      ...
      {(project.assumptions?.length ?? 0) > 0 && (
        <Card tone="sunken">
          <div className="flex flex-col gap-1" data-testid="assistant-assumptions">
      ...
      <Card>
        <div className="flex flex-col gap-4" data-testid="assistant-items">
          {gruposComItens.map((grupo) => (
            ...
            if (editando === id && campos) {
              return (
                <div key={id} className="flex flex-col gap-2" data-testid={`assistant-edit-${item.kind}-${item.key}`} style={{ padding: 10, borderRadius: 10, background: 'var(--surface-sunken)' }}>
                  {campos.map((campo) => (
                    <label key={campo.nome} className="flex flex-col gap-1" style={{ fontSize: 12, color: 'var(--text-muted)' }}>
                      ...
                      {campo.linhas ? (
                        <Textarea
                          ...
                          data-testid={`assistant-edit-field-${campo.nome}`}
                        />
                      ) : (
                        <Input
                          value={rascunho[campo.nome] ?? ''}
                  ...
                  {erroEdicao && (
                    <p style={{ fontSize: 12.5, color: 'var(--intent-danger-text)' }} data-testid="assistant-edit-error">
                      {erroEdicao}
                    </p>
                  )}
                  <div className="flex flex-wrap gap-2">
                    <Button onClick={() => salvar({ kind: item.kind, key: item.key, fields: rascunho })} disabled={salvando} data-testid="assistant-edit-save">
                      Salvar
                    </Button>
                    ...
                    <Button variant="secondary" onClick={() => salvar({ kind: item.kind, key: item.key, remove: true })} disabled={salvando} data-testid="assistant-edit-remove">
                      Tirar da proposta
                    </Button>
                  </div>
                </div>
            ...
            return (
              <div
                ...
                data-testid={`assistant-item-${item.kind}-${item.key}`}
              ...
              >
                ...
                <div style={{ minWidth: 0, flex: 1 }}>
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {item.usesLlm && (
                      <span style={{ fontSize: 11.5, color: 'var(--text-muted)', display: 'inline-flex', gap: 4, alignItems: 'center' }} data-testid="assistant-uses-llm">
                  ...
                  {item.rationale && (
                    <p style={{ fontSize: 12.5, color: 'var(--text-muted)', overflowWrap: 'anywhere' }} data-testid={`assistant-rationale-${item.kind}-${item.key}`}>
                ...
                {podeEditar && (
                  <IconButton
                    ...
                    onClick={() => abrir(item)}
                    disabled={carregando}
                    data-testid={`assistant-item-edit-${item.kind}-${item.key}`}
                  />
                )}
              </div>
            )
