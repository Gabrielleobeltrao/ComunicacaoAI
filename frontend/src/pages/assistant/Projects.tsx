...
import { createProject, deleteProject, listProjects } from '../../lib/assistant'
import type { AssistantProject } from '../../lib/assistant'
...
export function AssistantProjects() {
  ...
  const [projetos, setProjetos] = useState<AssistantProject[] | null>(null)
  /**
  ...
  const [aApagar, setAApagar] = useState<AssistantProject | null>(null)
  ...
  return (
    <AppLayout
      current="/assistant"
      title="Assistente · Montar operação"
      subtitle="Descreva o resultado que você quer. O Assistente faz as perguntas e propõe a estrutura."
    >
      <div className="flex flex-col gap-4" data-testid="assistant-projects">
        <Card>
          <div className="flex flex-col gap-3">
            <label htmlFor="assistant-objetivo" style={{ fontSize: 13, fontWeight: 600 }}>
              O que você quer que aconteça?
            </label>
            <Textarea
              id="assistant-objetivo"
              data-testid="assistant-objective"
              rows={3}
              value={objetivo}
            ...
            {erro && (
              <p role="alert" style={{ color: 'var(--intent-danger-text)', fontSize: 13 }} data-testid="assistant-error">
                {erro}
              </p>
            )}
            <div>
              <Button onClick={criar} disabled={!objetivo.trim() || criando} data-testid="assistant-start">
        ...
        {projetos === null ? (
          ...
          <EmptyState icon="sparkles" title="Nenhuma operação montada ainda" body="Descreva acima o que você quer e o Assistente começa a perguntar." />
        ) : (
          <div className="flex flex-col gap-2" data-testid="assistant-list">
            {projetos.map((p) => (
              ...
              >
                <button
                  type="button"
                  data-testid={`assistant-project-${p.id}`}
                  onClick={() => navigate(`/assistant/${p.id}`)}
                ...
                <IconButton
                  ...
                  data-testid={`assistant-delete-${p.id}`}
                />
              </div>
            ))}
          </div>
        ...
        <Dialog
          ...
          footer={
            <div className="flex flex-wrap justify-end gap-2">
              ...
              <Button onClick={() => void apagar()} disabled={apagando} data-testid="assistant-delete-confirm">
        ...
        >
          ...
          {erro && (
            <p role="alert" style={{ color: 'var(--intent-danger-text)', fontSize: 13 }} data-testid="assistant-delete-error">
              {erro}
            </p>
          )}
        </Dialog>
