...
import type { AssistantProject, ChecklistItem } from '../../lib/assistant'
...
export function Checklist({
  ...
  if (!itens.length) {
    return (
      <Card>
        <p style={{ fontSize: 13, color: 'var(--text-muted)' }} data-testid="assistant-no-checklist">
  ...
  return (
    <div className="flex flex-col gap-3" data-testid="assistant-checklist">
      <Card>
        <div className="flex flex-col gap-2">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <strong style={{ fontSize: 13 }}>Obrigatórios</strong>
            <span style={{ fontSize: 12, color: 'var(--text-muted)' }} data-testid="assistant-required-progress">
              {r.requiredDone}/{r.requiredTotal}
            </span>
          </div>
          <ProgressBar value={r.requiredTotal ? (r.requiredDone / r.requiredTotal) * 100 : 0} />
          {r.optionalTotal > 0 && (
            <p style={{ fontSize: 12, color: 'var(--text-muted)' }} data-testid="assistant-optional-progress">
              Opcionais: {r.optionalDone}/{r.optionalTotal}
            </p>
          )}
          <Badge tone={r.ready ? 'success' : 'warning'} data-testid="assistant-ready">
          ...
          <Card key={n}>
            <div className="flex flex-col gap-2">
              ...
              {grupo.map((item) => {
                ...
                return (
                  <div key={item.id} className="flex items-start gap-2" data-testid={`assistant-check-${item.id}`}>
                    ...
                    <div style={{ minWidth: 0, flex: 1 }}>
                      ...
                      <div className="flex flex-wrap items-center gap-2" style={{ marginTop: 4 }}>
                        ...
                        {(item.actionPath || alvo) && (
                          <a href={item.actionPath ?? alvo!.path} style={{ fontSize: 12.5, color: 'var(--intent-brand)' }} data-testid={`assistant-check-link-${item.id}`}>
                            Abrir
                          </a>
                        )}
                        {item.completionMode === 'manual' ? (
                          <button
                            type="button"
                            data-testid={`assistant-check-toggle-${item.id}`}
                          ...
                          <span style={{ fontSize: 11.5, color: 'var(--text-faint)' }} data-testid={`assistant-check-auto-${item.id}`}>
      ...
      <div>
        <Button variant="secondary" onClick={onReconferir} disabled={carregando} data-testid="assistant-recheck">
          Reconferir
        </Button>
      </div>
    </div>
