...
import * as arq from '../lib/assistant'
import type { AssistantMessage, AssistantProject, AssistantQuestion } from '../lib/assistant'
import { useSession } from '../lib/auth-client'

// O ASSISTENTE COMO CHAT GLOBAL — uma instância só, montada no layout.
//
// Antes, "Montar operação" morava dentro do seletor de andares: o único caminho para o
// Assistente estava atrás de um menu que a pessoa precisava saber abrir. E dentro do projeto
...
interface Mensagem {
  id: string
  /**
   * `aviso` é o `system_notice` da linha gravada — não é o Assistente falando, é o SISTEMA.
   *
   * Ele carrega coisas como "removi o que parecia uma credencial da sua mensagem" e a
   * falha de uma aplicação. Desenhá-lo como fala do Assistente apagaria a diferença entre
   * o que o modelo disse e o que o servidor fez.
   */
  autor: 'pessoa' | 'assistente' | 'aviso'
  /**
  ...
  resolvido?: boolean
  texto: string
  /** Uma pergunta curta do Assistente, quando os dois caminhos eram plausíveis. */
  pergunta?: string
...
interface AssistantState {
   ...
   * para falar de outra coisa era preciso descobrir a tela `/assistant` e apagar o projeto.
...
export const useAssistant = (): AssistantState | null => useContext(Ctx)
...
const LARGURA_CHAVE = 'comunicacaoai.assistant.width'
...
function ConfirmacaoPendente({ mensagemId, pendente }: { mensagemId: string; pendente: NonNullable<Mensagem['pendente']> }) {
  ...
  const campoId = `assistant-confirmar-nome-${mensagemId}`
  ...
  return (
    <div style={{ margin: '8px 0 0' }} data-testid="assistant-confirmar-operacao">
      ...
      {pendente.requiresName ? (
        ...
        >
          <Input
            ...
            data-testid="assistant-confirmar-nome"
            autoComplete="off"
            aria-required="true"
          />
        </Field>
      ...
      <Button
        ...
        data-testid="assistant-confirmar"
      >
        Confirmar
      </Button>
    </div>
...
export function AssistantProvider({ children }: { children: ReactNode }) {
  ...
  const [pergunta, setPergunta] = useState<AssistantQuestion | null>(null)
  ...
  const daLinha = (m: AssistantMessage): Mensagem => ({
    id: m.id,
    autor: m.role === 'user' ? 'pessoa' : m.role === 'system_notice' ? 'aviso' : 'assistente',
    texto: m.content,
  ...
  const enviar = useCallback(async () => {
     ...
     * a aplicação. Continuar mandando para a porta de entrada faria o Assistente reclassificar
    ...
    try {
      const res = await fetch(`${API_URL}/api/assistant/assistant/turn`, {
        method: 'POST',
        credentials: 'include',
  ...
  const confirmar = useCallback(async (mensagemId: string, nomeDigitado?: string) => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/assistant/assistant/confirm`, {
        method: 'POST',
        credentials: 'include',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
  ...
  const idNaUrl = useMemo(() => {
    const m = location.pathname.match(/^\/assistant\/([^/]+)/)
    // `/assistant/new` é redirecionamento antigo, e não um id.
  ...
  return (
    <Ctx.Provider value={valor}>
      ...
      {!sessao?.user ? null : (
        <>
          <AssistantLauncher />
          <AssistantPanel onAbrirProjeto={(id) => navigate(`/assistant/${id}`)} />
        </>
      )}
    </Ctx.Provider>
  )
...
function AssistantLauncher() {
   ...
   * clicava e abria o Assistente. Um modal é dono da tela enquanto está aberto.
   *
   ...
   */
  useEffect(() => {
    // O painel do próprio Assistente tem `role="dialog"` e NÃO conta: contá-lo faria o botão
    // sumir para sempre depois da primeira abertura.
    const conferir = () => setTemModal(document.querySelector('[role="dialog"]:not([data-testid="assistant-panel"])') !== null)
    conferir()
...
const ehPaginaDeProjeto = (pathname: string) => /^\/assistant\/[^/]+/.test(pathname)

function AssistantPanel({ onAbrirProjeto }: { onAbrirProjeto: (id: string) => void }) {
  const a = useAssistant()
   ...
   * fica exatamente ali. Quem precisa de espaço lê `--assistente-largura`; no celular ela é
  ...
  useEffect(() => {
    ...
    const aplicar = () => {
      ...
      raiz.style.setProperty('--assistente-largura', larguraAberta && cabe ? `${largura}px` : '0px')
    }
    aplicar()
    window.addEventListener('resize', aplicar)
    return () => {
      window.removeEventListener('resize', aplicar)
      raiz.style.setProperty('--assistente-largura', '0px')
    }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <strong style={{ fontSize: 14, flex: 1 }}>Assistente</strong>
        {a.phase !== 'idle' && PHASE_LABEL[a.phase] ? (
          <span data-testid="assistant-phase" style={{ fontSize: 12, color: 'var(--text-muted)' }}>
            {PHASE_LABEL[a.phase]}
          </span>
        ) : null}
        ...
        {!naPaginaDeProjeto && (a.mensagens.length > 0 || a.projeto) ? (
          <button
            ...
            data-testid="assistant-nova-conversa"
            style={botaoDeIcone}
          >
            <Icon name="plus" size={16} />
          </button>
        ) : null}
        <button type="button" onClick={a.fechar} aria-label="Fechar o Assistente" data-testid="assistant-close" style={botaoDeIcone}>
          <Icon name="x" size={16} />
        </button>
      </header>
          ...
          */}
          ...
          >
            {a.mensagens.length === 0 ? (
              <p style={{ margin: 0, fontSize: 13, color: 'var(--text-muted)' }} data-testid="assistant-vazio">
            ...
            {a.mensagens.map((m) => (
              ...
              <div
                ...
                data-testid={`assistant-message-${m.autor === 'pessoa' ? 'user' : m.autor === 'aviso' ? 'system_notice' : 'assistant'}`}
              ...
              >
                ...
                {m.pergunta ? (
                  <p style={{ margin: '6px 0 0', fontSize: 13, fontWeight: 600 }} data-testid="assistant-question">
                    {m.pergunta}
                  </p>
                ) : null}
                {m.pendente ? <ConfirmacaoPendente mensagemId={m.id} pendente={m.pendente} /> : null}
                {m.desfecho ? (
                  <p style={{ margin: '8px 0 0', fontSize: 12.5, color: 'var(--text-muted)' }} data-testid="assistant-desfecho">
                    {m.desfecho}
                  </p>
                ) : null}
            ...
            {a.idDaProposta === m.id && a.projeto?.hasBlueprint && !naPaginaDeProjeto ? (
              <p style={{ margin: '2px 0 0', alignSelf: 'flex-start' }}>
                <Button variant="secondary" onClick={() => onAbrirProjeto(a.projeto!.id)} data-testid="assistant-abrir-projeto">
                  Abrir a proposta
                </Button>
              </p>
            ) : null}
            ...
            {a.enviando ? (
              <p role="status" data-testid="assistant-thinking" style={{ margin: 0, fontSize: 12.5, color: 'var(--text-muted)' }}>
                Pensando… {segundos}s
              </p>
            ) : null}
            <div ref={fim} />
          ...
          {a.pergunta && !a.enviando ? (
            <div className="flex flex-col gap-2" style={{ padding: '0 12px 8px' }} data-testid="assistant-question">
              ...
              {(a.pergunta.choices ?? []).map((c) => (
                <Button key={c.value} variant="secondary" size="sm" onClick={() => void a.responder(c.label)} data-testid={`assistant-choice-${c.value}`}>
                  {c.label}
                </Button>
              ))}
              {a.pergunta.allowUnknown ? (
                <Button variant="ghost" size="sm" onClick={() => void a.responder('Não sei ainda')} data-testid="assistant-unknown">
                  Não sei ainda
                </Button>
              ) : null}
              </div>
          ...
          {a.projeto && !a.projeto.hasBlueprint && !a.enviando ? (
            <p style={{ margin: 0, padding: '0 12px 8px' }}>
              <Button variant="ghost" size="sm" onClick={() => void a.gerarProposta()} data-testid="assistant-generate">
          ...
          {a.erro ? (
            <p role="alert" data-testid="assistant-erro" style={{ margin: 0, padding: '0 12px 8px', fontSize: 12.5, color: 'var(--intent-danger-text)' }}>
              {a.erro}
            </p>
          ) : null}
          ...
          >
            ...
            />
            <Button type="submit" disabled={!a.rascunho.trim() || trabalhando} data-testid="assistant-enviar">
              {trabalhando ? '…' : 'Enviar'}
            </Button>
          </form>
        </>
