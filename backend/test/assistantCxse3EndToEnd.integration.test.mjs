...
// o sistema entendeu.
//
// Aqui a única entrada é `POST /api/assistant/assistant/turn` com a frase que alguém digitaria.
...
test('os recursos criados levam a MARCA da operação — a retomada os reconhece', async () => {
  ...
  for (const [colecao, minimo] of [['data_stores', 1], ['automations', 1], ['monitors', 0], ['monitoring_sources', 0]]) {
    ...
    for (const doc of docs) {
      assert.equal(doc.assistant?.operationId, operacao._id.toString(), `sem marca, uma queda na janela duplicaria este ${colecao}`)
      assert.ok(doc.assistant?.blueprintKey, `a marca sem a chave do plano não reconhece O QUE é este ${colecao}`)
    }
  }
})
