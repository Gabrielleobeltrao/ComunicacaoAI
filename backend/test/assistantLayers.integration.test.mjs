...
const { runLlmCritique } = await import('../dist/assistant/criticLlm.js')
...
beforeEach(async () => {
  for (const c of ['assistant_projects', 'assistant_messages', 'assistant_operations', 'floors', 'agents', 'sectors', 'automations', 'buildings', 'user_settings', 'token_usage', 'token_usage_charges']) {
...
async function projetoCom(blueprint, extra = {}) {
  ...
  const { selectLayer } = await import('../dist/assistant/compile.js')
...
test('corrigir "O que entendi" refaz a proposta — sem chamar o modelo', async () => {
  const { emptyBrief, applyBriefPatch } = await import('../dist/assistant/brief.js')
  const { compileBrief } = await import('../dist/assistant/compile.js')
...
test('projeto legado NÃO é recompilado quando o entendimento muda', async () => {
  ...
  const p = await projetoCom(LEGADO(), { brief: { ...(await import('../dist/assistant/brief.js')).emptyBrief('atender'), businessGoal: 'x' } })
