// AS ROTAS do Assistente, com o Express de verdade e o dublê no lugar do modelo.
//
...
const { ensureAssistantIndexes } = await import('../dist/assistant/repository.js')
...
const VIZINHO = 'vizinho-assistente'
let server
let port
let sessao = DONO
...
beforeEach(async () => {
  for (const c of ['assistant_projects', 'assistant_messages', 'assistant_apply_operations', 'token_usage', 'token_usage_charges', 'user_settings', 'agents', 'sectors', 'offices', 'buildings', 'app_installations'])
...
test('apagar remove a conversa e as mensagens — e nada além disso', async () => {
  ...
  assert.ok((await db.collection('assistant_messages').countDocuments({})) > 0, 'a mensagem foi gravada')

  const r = await pedir('DELETE', `/projects/${p.id}`)
  assert.equal(r.status, 204)
  assert.equal(await db.collection('assistant_projects').countDocuments({}), 0)
  assert.equal(await db.collection('assistant_messages').countDocuments({}), 0, 'as mensagens vão junto')
...
test('não dá para apagar um projeto que está sendo aplicado', async () => {
  ...
  await db.collection('assistant_projects').updateOne({ _id: new ObjectId(p.id) }, { $set: { status: 'applying' } })

  const r = await pedir('DELETE', `/projects/${p.id}`)
  assert.equal(r.status, 409, `devolveu ${r.status}`)
  assert.equal(await db.collection('assistant_projects').countDocuments({}), 1, 'continua lá')
})
...
test('o ensaio fica guardado no projeto, versionado', async () => {
  const p = await comProposta()
  const doc = await db.collection('assistant_projects').findOne({ _id: new ObjectId(p.id) })
