...
const cap = await import('../dist/assistant/assistantCapabilities.js')
const op = await import('../dist/assistant/assistantOperate.js')
...
beforeEach(async () => {
  for (const c of ['monitoring_sources', 'live_data', 'assistant_pending_operations', 'buildings', 'offices', 'agents', 'user_settings', 'audit_events'])
...
test('AMEAÇA: a operação EXPIRADA não executa', async () => {
  const { fonte, turno } = await prepararPausa()
  await db.collection('assistant_pending_operations').updateOne({ id: turno.pendingOperation.id }, { $set: { expiresAt: new Date(Date.now() - 1000) } })
...
test('CARACTERIZAÇÃO: a operação guarda o RESULTADO da execução, e não só a confirmação', async () => {
  ...
  const doc = await db.collection('assistant_pending_operations').findOne({ id: turno.pendingOperation.id })
...
test('AMEAÇA: o handler que ESTOURA não vira sucesso — e a operação diz que falhou', async () => {
   ...
   */
  const capMod = await import('../dist/assistant/assistantCapabilities.js')
  const alvo = await db.collection('assistant_pending_operations').findOne({ id: turno.pendingOperation.id })
  ...
  const doc = await db.collection('assistant_pending_operations').findOne({ id: alvo.id })
...
test('AUDITORIA: a tentativa que ESTOURA no handler também deixa linha', async () => {
  ...
  try {
    ...
    const r = await fetch(`http://127.0.0.1:${porta}/api/assistant/assistant/confirm`, {
      method: 'POST',
    ...
    const linhas = await db.collection('audit_events').find({ entityType: 'assistant_operation' }).toArray()
