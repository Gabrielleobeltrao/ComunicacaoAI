...
//
// No V1 isso é fechado pela MARCA DE ORIGEM — a retomada procura por `assistant.operationId`
...
test('ACEITAÇÃO: o recurso criado leva a MARCA da operação que o criou', async () => {
  ...
  assert.equal(base.assistant?.operationId, operationId, 'sem a marca, a retomada não acha o que ficou de pé')
  assert.equal(base.assistant?.blueprintKey, 'base')

  const fonte = await db.collection('monitoring_sources').findOne({ ownerId: DONO })
  assert.equal(fonte.assistant?.operationId, operationId)
  assert.equal(fonte.assistant?.blueprintKey, 'fonte')
})
...
for (const [kind, colecao] of Object.entries(COLECAO_DE)) {
  test(`ACEITAÇÃO: o ${kind} criado leva a marca da operação`, async () => {
    ...
    assert.equal(doc.assistant?.operationId, operationId, `o ${kind} não levou a marca: a retomada não o acharia`)
    assert.equal(doc.assistant?.projectId, projectId)
    assert.ok(doc.assistant?.blueprintKey, `o ${kind} não levou a key do plano`)
  })
...
test('sem operationId, nada muda: a marca é opcional', async () => {
  ...
  const base = await db.collection('data_stores').findOne({ ownerId: DONO })
  assert.equal(base.assistant, undefined, 'quem não passa operationId não ganha marca')
})
