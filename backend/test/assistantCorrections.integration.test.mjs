...
const repo = await import('../dist/assistant/repository.js')
const service = await import('../dist/assistant/service.js')
const { computeBlueprintHash } = await import('../dist/assistant/blueprint.js')
const { validateOfficeBlueprint } = await import('../dist/assistant/validate.js')
const { loadOwnershipContext } = await import('../dist/assistant/context.js')
const { semResourceId } = await import('../dist/assistant/turn.js')
...
const { resetGuards } = await import('../dist/assistant/guard.js')
...
const pedir = async (metodo, caminho, corpo) => {
  const res = await fetch(`http://127.0.0.1:${port}/api/assistant${caminho}`, {
    method: metodo,
...
before(async () => {
  ...
  await repo.ensureAssistantIndexes()
...
async function projetoCom(blueprint) {
  ...
  const { deriveChecklist, applyChecklistState, computeReadiness } = await import('../dist/assistant/checklist.js')
...
test('o que a aplicação cria leva a marca de origem', async () => {
  ...
  for (const [colecao, key] of [['offices', 'andar'], ['agents', 'gerente'], ['sectors', 'setor']]) {
    const doc = await db.collection(colecao).findOne({ ownerId: DONO, 'assistant.blueprintKey': key })
    assert.ok(doc, `${colecao} sem marca`)
    assert.equal(doc.assistant.operationId, operationId)
    assert.equal(doc.assistant.projectId, p._id.toString())
  }
})
...
test('o teto é conferido antes do REPARO também', async () => {
  const { runAssistantTurn } = await import('../dist/assistant/turn.js')
  const { recordReplyUsage } = await import('../dist/tokenUsage.js')
  await setMonthlyTokenCap(DONO, 40)

  // Prompt sem a marca do Assistente: a primeira resposta é ilegível e o reparo entra.
  const r = await runAssistantTurn({ ownerId: DONO, provider: 'anthropic', model: null, prompt: 'x'.repeat(400), chargeKey: 'teto' })
...
test('gerar e revisar ficam no log; a conversa não', async () => {
  ...
  assert.deepEqual(auditTargetFor('POST', `/api/assistant/projects/${ID}/generate`), { entityType: 'assistant_project', entityId: ID, action: 'update' })
  assert.deepEqual(auditTargetFor('POST', `/api/assistant/projects/${ID}/validate`), { entityType: 'assistant_project', entityId: ID, action: 'test' })
  assert.deepEqual(auditTargetFor('POST', `/api/assistant/projects/${ID}/apply`), { entityType: 'assistant_project', entityId: ID, action: 'publish' })
  assert.deepEqual(auditTargetFor('POST', `/api/assistant/projects/${ID}/resume`), { entityType: 'assistant_project', entityId: ID, action: 'publish' })
  assert.deepEqual(auditTargetFor('POST', `/api/assistant/projects/${ID}/rollback`), { entityType: 'assistant_project', entityId: ID, action: 'delete' })
  assert.equal(auditTargetFor('POST', `/api/assistant/projects/${ID}/messages`), null)
})
...
const t2 = await import('../dist/assistant/typesV2.js')
