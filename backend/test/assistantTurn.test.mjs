...
process.env.MONGODB_URI = 'mongodb://127.0.0.1:27017/assistant-turn-test'
process.env.BETTER_AUTH_SECRET = process.env.BETTER_AUTH_SECRET ?? 'test-secret'

const { extractJson, normalizeTurn } = await import('../dist/assistant/turn.js')
const { buildAssistantPrompt, buildRepairPrompt, ASSISTANT_MARKER } = await import('../dist/assistant/prompt.js')
...
test('o prompt proíbe inventar fato e escrever credencial', () => {
  const prompt = buildAssistantPrompt({ project: projeto, messages: [], apps: [] })
...
test('respostas já registradas entram para a pergunta não se repetir', () => {
  const prompt = buildAssistantPrompt({ project: { ...projeto, answers: { canais: 'site e whatsapp' } }, messages: [], apps: [] })
...
test('o reparo pede JSON e nada mais, uma vez só', () => {
  ...
  assert.ok(p.startsWith(ASSISTANT_MARKER))
})
...
test('o dublê está ligado no teste e responde a jornada determinística', async () => {
  ...
  const primeiro = await askAuxWithUsage('anthropic', buildAssistantPrompt({ project: projeto, messages: [{ role: 'user', content: 'quero automatizar o atendimento' }], apps: [] }))
  ...
  const segundo = await askAuxWithUsage(
    'anthropic',
    buildAssistantPrompt({ project: { ...projeto, answers: { 'canais-de-atendimento': 'site' } }, messages: [{ role: 'user', content: 'site' }], apps: [] }),
  )
...
test('o dublê não responde a prompt que não é do Assistente', async () => {
