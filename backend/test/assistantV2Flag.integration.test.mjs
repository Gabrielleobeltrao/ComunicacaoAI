...
// atendidos, então o padrão virou o V2 — mas `ASSISTANT_BLUEPRINT_V2=0` continua existindo, e
...
const { assistantV2Enabled } = await import('../dist/assistant/flags.js')
...
beforeEach(async () => {
  for (const c of ['assistant_projects', 'assistant_messages', 'assistant_apply_operations', 'agents', 'sectors', 'offices', 'buildings', 'automations', 'data_stores', 'monitoring_sources', 'monitors'])
...
test('a flag nasce LIGADA, e só um "não" explícito a desliga', () => {
  delete process.env.ASSISTANT_BLUEPRINT_V2
  assert.equal(assistantV2Enabled(), true, 'os critérios de saída foram atendidos: o padrão é o V2')
  ...
  for (const v of ['0', 'false', 'OFF']) {
    process.env.ASSISTANT_BLUEPRINT_V2 = v
    assert.equal(assistantV2Enabled(), false, `"${v}" tinha que desligar`)
  }
  for (const v of ['1', 'true', 'on', 'talvez', '']) {
    process.env.ASSISTANT_BLUEPRINT_V2 = v
    assert.equal(assistantV2Enabled(), true, `"${v}" não pode desligar`)
  }
  delete process.env.ASSISTANT_BLUEPRINT_V2
})
