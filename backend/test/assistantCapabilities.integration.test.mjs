// O CATÁLOGO VIVO: o que o Assistente pode propor é o que o sistema tem.
//
...
const { buildCapabilityManifest, manifestForPrompt } = await import('../dist/assistant/capabilities.js')
...
test('as capacidades de cada perfil vêm do MESMO resolvedor que o runtime usa', async () => {
  ...
  for (const p of m.presets) {
    ...
    // aparece como "o Assistente propôs algo que o agente não consegue fazer".
...
test('Fase 4: o catálogo cita MONITOR e FONTE — e diz o que o Assistente NÃO faz', async () => {
  /**
...
test('o resumo do inventário mostra O QUE TEM DENTRO de um conjunto, não só o nome', async () => {
   ...
   */
  const { summarizeInventory } = await import('../dist/assistant/inventory.js')
