...
const { buildAssistantPrompt } = await import('../dist/assistant/prompt.js')
...
const DONO = 'dono-assistente-turno'
...
test('sem chave de provedor, o Assistente recusa em vez de tentar', async () => {
  const r = await runAssistantTurn({ ownerId: DONO, provider: 'anthropic', model: null, prompt: prompt(), chargeKey: 'k1' })
...
test('o limite mensal é conferido ANTES da chamada', async () => {
  ...
  const r = await runAssistantTurn({ ownerId: DONO, provider: 'anthropic', model: null, prompt: prompt(), chargeKey: 'k2' })
  assert.equal(r.ok, false)
...
test('a rodada cobra, e a MESMA rodada repetida não cobra de novo', async () => {
  ...
  const primeira = await runAssistantTurn({ ownerId: DONO, provider: 'anthropic', model: null, prompt: prompt(), chargeKey: 'rodada-1' })
  ...
  const repetida = await runAssistantTurn({ ownerId: DONO, provider: 'anthropic', model: null, prompt: prompt(), chargeKey: 'rodada-1' })
  ...
  const outra = await runAssistantTurn({ ownerId: DONO, provider: 'anthropic', model: null, prompt: prompt(), chargeKey: 'rodada-2' })
...
test('resposta ilegível falha de forma segura — e o que já foi gasto fica contabilizado', async () => {
  await setProviderApiKey(DONO, 'anthropic', 'chave-de-teste')
  // Um prompt sem a marca do Assistente: o dublê devolve vazio, que não é JSON.
  const r = await runAssistantTurn({ ownerId: DONO, provider: 'anthropic', model: null, prompt: 'sem a marca', chargeKey: 'ilegivel' })
  assert.equal(r.ok, false)
...
test('a falha não vaza mensagem de provedor para a tela', async () => {
  await setProviderApiKey(DONO, 'anthropic', 'chave-de-teste')
  const r = await runAssistantTurn({ ownerId: DONO, provider: 'anthropic', model: null, prompt: 'sem a marca', chargeKey: 'ilegivel-2' })
  assert.equal(r.ok, false)
...
test('ACEITAÇÃO: com só OpenAI configurada, o turno USA a OpenAI — mesmo com o projeto marcado anthropic', async () => {
  ...
  const r = await runAssistantTurn({ ownerId: DONO, provider: 'anthropic', model: null, prompt: prompt(), chargeKey: 'prov-1' })
...
test('AMEAÇA: sem chave NENHUMA, a recusa diz quais provedores ele procurou', async () => {
   ...
   */
  const r = await runAssistantTurn({ ownerId: DONO, provider: 'anthropic', model: null, prompt: prompt(), chargeKey: 'prov-2' })
  assert.equal(r.ok, false)
  assert.equal(r.failure.code, 'no_provider_key')
