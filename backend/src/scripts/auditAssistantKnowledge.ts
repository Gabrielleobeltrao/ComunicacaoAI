...
import { auditAssistantMemoryMigration } from '../knowledgeMigration.js'

// A CONFERÊNCIA da mudança de casa — só leitura.
//
// `node dist/scripts/auditAssistantKnowledge.js <tenantId>`. Mostra, para cada memória
// do Assistente, onde ela foi parar e se a cópia está lá para ser lida. Não apaga nada e
// não escreve nada: a limpeza é um comando à parte, e ele vai precisar desta lista.
const tenantId = process.argv[2]
if (!tenantId) {
  console.error('uso: auditAssistantKnowledge <tenantId>')
  process.exit(2)
}

await mongoClient.connect()
const r = await auditAssistantMemoryMigration(tenantId)
