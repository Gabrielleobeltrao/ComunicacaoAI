...
import { readConnectionConfig, writeConnectionConfig } from '../integrations/websocket/service.js'
import {
  deleteSubscription,
  findSubscription,
  insertSubscription,
...
import { ensureStream, pauseStream, removeStream, resumeStream } from '../streams/service.js'
...
websocketRouter.post('/subscriptions', async (req, res, next) => {
  try {
    ...
    const doc = await insertSubscription({
      ...
      subscribeMessage: texto(body.subscribeMessage, 4_000),
      unsubscribeMessage: texto(body.unsubscribeMessage, 4_000),
    ...
    res.status(201).json(subscriptionPublic(doc))
  } catch (error) {
    fail(res, error, next)
  }
})
...
websocketRouter.patch('/subscriptions/:id', async (req, res, next) => {
  try {
    ...
    if (body.destination !== undefined) set.destination = normalizeDestination(body.destination)
    ...
    res.json(subscriptionPublic(atualizada))
  } catch (error) {
    fail(res, error, next)
  }
})
...
websocketRouter.delete('/subscriptions/:id', async (req, res, next) => {
  try {
    const id = oid(req.params.id)
    if (!id || !(await deleteSubscription(res.locals.userId, id))) return notFound(res)
    res.status(204).end()
  } catch (error) {
    fail(res, error, next)
  }
