...
import { AssistantRefusal } from '../assistant/service.js'
import { ApplyFailure } from '../assistant/apply.js'
import { allowRate, withProjectLock } from '../assistant/guard.js'
import * as L from '../assistant/limits.js'
import { auditEntity } from './auditMiddleware.js'
import { runAssistantTurn, resolveUiContext, ASSISTANT_LIMITS } from '../assistant/assistant.js'
import { loadOfficeInventory, summarizeInventory } from '../assistant/inventory.js'
...
function refuse(res: Parameters<typeof fail>[0], error: unknown, next: Parameters<typeof fail>[2]): void {
  ...
  if (error instanceof AssistantRefusal) {
...
const comRitmo = (ownerId: string): void => {
  if (!allowRate(ownerId, 10, 60_000)) throw new AssistantRefusal('too_many_messages', 'muitas mensagens em pouco tempo; aguarde um instante')
}
 ...
 */
assistantRouter.post('/assistant/turn', async (req, res, next) => {
  ...
  try {
     ...
     * flutuante não pode ficar sem registro só porque não passou pela tela do Assistente.
     */
    if (r.projectId) {
      await recordAudit({
        ownerId: res.locals.userId,
 ...
 */
assistantRouter.post('/assistant/confirm', async (req, res, next) => {
  const b = (req.body ?? {}) as { id?: unknown; operationHash?: unknown; confirmationName?: unknown }
  try {
    const { confirmarOperacao } = await import('../assistant/assistantOperate.js')
    ...
    await recordAudit({
      ...
      actorId: res.locals.userId,
      action: 'update',
      entityType: 'assistant_operation',
...
assistantRouter.get('/context', async (req, res, next) => {
  try {
...
assistantRouter.post('/projects', async (req, res, next) => {
  try {
...
assistantRouter.get('/projects', async (req, res) => {
...
assistantRouter.get('/projects/:id', async (req, res) => {
...
assistantRouter.patch('/projects/:id', async (req, res, next) => {
  const id = oid(req.params.id)
  if (!id) return notFound(res)
  try {
...
assistantRouter.patch('/projects/:id/blueprint', async (req, res, next) => {
  const id = oid(req.params.id)
  if (!id) return notFound(res)
  try {
...
assistantRouter.patch('/projects/:id/brief', async (req, res, next) => {
  const id = oid(req.params.id)
  if (!id) return notFound(res)
  try {
...
assistantRouter.get('/targets', async (_req, res) => {
  res.json(await service.assistantTargets(res.locals.userId))
})

assistantRouter.patch('/projects/:id/links', async (req, res, next) => {
  const id = oid(req.params.id)
  if (!id) return notFound(res)
  try {
...
assistantRouter.patch('/projects/:id/layer', async (req, res, next) => {
  const id = oid(req.params.id)
  if (!id) return notFound(res)
  try {
...
assistantRouter.get('/projects/:id/messages', async (req, res) => {
  const id = oid(req.params.id)
  if (!id) return notFound(res)
  const projeto = await repo.getProject(res.locals.userId, id)
  if (!projeto) return notFound(res)
...
assistantRouter.post('/projects/:id/messages', async (req, res, next) => {
  const id = oid(req.params.id)
  if (!id) return notFound(res)
  try {
...
assistantRouter.post('/projects/:id/turn', async (req, res, next) => {
  const id = oid(req.params.id)
  if (!id) return notFound(res)
  try {
    comRitmo(res.locals.userId)
...
assistantRouter.post('/projects/:id/generate', async (req, res, next) => {
  const id = oid(req.params.id)
  if (!id) return notFound(res)
  try {
    comRitmo(res.locals.userId)
...
assistantRouter.post('/projects/:id/validate', async (req, res, next) => {
  const id = oid(req.params.id)
  if (!id) return notFound(res)
  try {
...
assistantRouter.get('/projects/:id/preview', async (req, res, next) => {
  const id = oid(req.params.id)
  if (!id) return notFound(res)
  try {
...
assistantRouter.patch('/projects/:id/checklist/:itemId', async (req, res, next) => {
  const id = oid(req.params.id)
  if (!id) return notFound(res)
  try {
...
assistantRouter.post('/projects/:id/archive', async (req, res, next) => {
  const id = oid(req.params.id)
  if (!id) return notFound(res)
  try {
...
assistantRouter.delete('/projects/:id', async (req, res, next) => {
  const id = oid(req.params.id)
  if (!id) return notFound(res)
  try {
...
// --- aplicação ---------------------------------------------------------------------
// Estas quatro são as únicas rotas do Assistente que escrevem no escritório — e a
// primeira delas exige hash revisado, chave de operação e confirmação explícita.

assistantRouter.post('/projects/:id/apply', async (req, res, next) => {
  const id = oid(req.params.id)
  if (!id) return notFound(res)
  try {
...
assistantRouter.post('/projects/:id/resume', async (req, res, next) => {
  const id = oid(req.params.id)
  if (!id) return notFound(res)
  try {
...
assistantRouter.post('/projects/:id/recheck', async (req, res, next) => {
  const id = oid(req.params.id)
  if (!id) return notFound(res)
  try {
...
assistantRouter.post('/projects/:id/rollback', async (req, res, next) => {
  const id = oid(req.params.id)
  if (!id) return notFound(res)
  try {
