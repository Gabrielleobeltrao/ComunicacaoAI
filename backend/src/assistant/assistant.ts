...
import type { AssistantIntent } from './intent.js'
 ...
 */
export async function resolveUiContext(ownerId: string, bruto: AssistantUiContext | null | undefined): Promise<ResolvedUiContext> {
  const ctx = bruto ?? {}
...
export interface AssistantTurnInput {
  ...
  /** Qual provedor classifica. O padrão é o mesmo que o Assistente usa nos projetos. */
...
async function executarCapacidade(
  ...
  try {
    ...
    console.error('[assistant] capacidade do assistente falhou:', (erro as Error)?.message)
...
async function responderEstatico(input: AssistantTurnInput, mensagem: string): Promise<string> {
  try {
    ...
    console.error('[assistant] resposta estática falhou:', (erro as Error)?.message)
    return ''
  }
}
 ...
 */
async function explicarRecurso(ownerId: string, intent: Extract<AssistantIntent, { mode: 'explain' }>, ctx: ResolvedUiContext): Promise<string> {
