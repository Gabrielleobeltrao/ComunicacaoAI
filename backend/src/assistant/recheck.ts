...
import { assistantKnowledgeExists } from './knowledge.js'
import type { KnowledgeScope } from './knowledge.js'
import type { AssistantChecklistItem, AssistantReadiness } from './types.js'
...
export async function recheckProject(ownerId: string, project: AssistantProject): Promise<{ checklist: AssistantChecklistItem[]; readiness: AssistantReadiness }> {
  ...
  const reescritos = new Map<string, Partial<AssistantChecklistItem>>()
  ...
  for (const item of itens) {
    ...
    if (alvo.kind === 'knowledge') {
      ...
      if (escopo && (await assistantKnowledgeExists(ownerId, escopo, id))) concluidos.add(item.id)
      continue
    }
...
): Promise<{ itens: AssistantChecklistItem[]; bloqueios: string[] }> {
...
const escopoDoConhecimento = (project: AssistantProject, key: string): KnowledgeScope | null =>
...
async function mapaDeRecursos(ownerId: string, project: AssistantProject): Promise<Map<string, string>> {
  const operacoes = await db
    .collection('assistant_apply_operations')
...
export async function appliedLinks(ownerId: string, project: AssistantProject): Promise<{ kind: string; key: string; id: string; path: string }[]> {
  const mapa = await mapaDeRecursos(ownerId, project)
