...
export interface AssistantTurnResult {
   ...
   * misturar os dois num artefato só foi o que fazia o Assistente inventar andar e
...
export type TurnOutcome = { ok: true; result: AssistantTurnResult; usage: TokenUsage } | { ok: false; failure: TurnFailure; usage: TokenUsage }
 ...
 */
export function normalizeTurn(bruto: unknown): AssistantTurnResult | null {
  ...
  const resultado: AssistantTurnResult = {
    assistantText,
    phase,
    question,
    answerPatch,
 ...
 * Assistente realmente não pode trabalhar.
 */
 ...
 */
export async function runAssistantTurn(input: RunTurnInput): Promise<TurnOutcome> {
  /**
  ...
  if (!escolha) {
    return {
      ...
      failure: {
        ...
        message: 'Configure em Configurações a chave da Anthropic ou da OpenAI para o Assistente poder trabalhar.',
      },
      usage: semUso(),
    }
  }
  ...
  const chamar = async (prompt: string, sufixoDaChave: string): Promise<{ text: string } | { erro: TurnFailure }> => {
    ...
    try {
      ...
      console.error(`[assistant] falha na chamada ao provedor (${motivo.code}):`, (error as Error).message)
      return { erro: motivo }
    }
  }
