...
import { writeAssistantKnowledge, deleteAssistantKnowledge } from './knowledge.js'
...
import type { AssistantProject, AssistantApplyOperation } from './repository.js'
...
// A APLICAÇÃO: a única parte do Assistente que escreve no escritório.
//
...
interface Contexto {
  ownerId: string
  operation: AssistantApplyOperation
...
async function recuperarPelaMarca(ctx: Contexto, kind: ApplyStepKind, key: string): Promise<string | null> {
  ...
  const doc = await db
    .collection(colecao)
    .findOne({ ownerId: ctx.ownerId, 'assistant.operationId': ctx.operation._id.toString(), 'assistant.blueprintKey': key }, { projection: { _id: 1 } })
  return doc ? doc._id.toString() : null
}

/** A marca gravada junto com o recurso, na mesma escrita. */
const marcaDe = (ctx: Contexto, key: string): AssistantStamp => ({
...
export async function applyBlueprint(
  ...
  hooks: ApplyHooks = {},
): Promise<AssistantApplyOperation> {
...
async function executarSaga(ctx: Contexto): Promise<void> {
  ...
  for (const floor of bp.floors ?? []) {
    await passo(ctx, 'floor', floor.key, async () => {
      ...
      const criado = await createFloor(ctx.ownerId, {
        ...
        assistant: marcaDe(ctx, floor.key),
      })
  ...
  for (const agent of bp.agents ?? []) {
    await passo(ctx, 'agent', agent.key, async () => {
      ...
      const criado = await createAgent(ctx.ownerId, new ObjectId(floorId), agent.name, { ...camposDoAgente(agent), assistant: marcaDe(ctx, agent.key) })
      return { id: criado._id.toString(), status: 'created' }
    })
  }
  ...
  for (const sector of bp.sectors ?? []) {
    await passo(ctx, 'sector', sector.key, async () => {
      ...
      const criado = await createSector(ctx.ownerId, new ObjectId(floorId), sector.name, sector.color ?? '#6366f1', sector.mode, membros, {
        assistant: marcaDe(ctx, sector.key),
  ...
  for (const req of bp.knowledgeRequirements ?? []) {
    ...
    await passo(ctx, 'knowledge', req.key, async () => {
      ...
      const r = await writeAssistantKnowledge(ctx.ownerId, req, conteudo, { resolve: (kind, key) => ctx.mapa.get(chave(kind as ApplyStepKind, key)) })
  ...
  for (const routine of bp.routines ?? []) {
    await passo(ctx, 'routine', routine.key, async () => {
      ...
      const criada = await createAutomation(ctx.ownerId, {
        floorId,
        assistant: marcaDe(ctx, routine.key),
        name: routine.name,
 ...
 */
export async function resumeApply(ownerId: string, project: AssistantProject, hooks: ApplyHooks = {}): Promise<AssistantApplyOperation> {
...
async function desfazerV2(
  ...
  const marca = (doc as { assistant?: { operationId?: string } }).assistant
  ...
  for (const kind of ordem) {
    for (const step of operacao.steps.filter((s) => s.kind === kind && s.status === 'created' && s.resourceId)) {
      ...
      if (kind === 'knowledge') {
        ...
        if (await deleteAssistantKnowledge(ownerId, escopo, id)) removed.push(`${kind}:${step.key}`)
        continue
      }
