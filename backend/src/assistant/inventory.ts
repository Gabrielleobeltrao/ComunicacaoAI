...
//
// O Assistente V1 montava o contexto à mão, escolhendo alguns campos de alguns domínios. Isso
...
export async function loadOfficeInventory(ownerId: string): Promise<OfficeInventory> {
  const [predio, andares, agentes, instalacoes, stores, fontes, monitores, flows, ferramentas, canais, conexoes] = await Promise.all([
    ...
    // CANAIS e ENTREGAS: sem eles, o Assistente propunha criar o canal que já existe e não
    ...
    for (const d of await listDatasets(ownerId, s._id).catch(() => [])) {
      datasets.push({
        ...
        meta: {
           ...
           *
           * Sem eles o Assistente recebia "Bitcoin" e não tinha como saber que ali dentro
