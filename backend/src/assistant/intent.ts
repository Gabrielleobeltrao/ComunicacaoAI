...
//
// O Assistente V1 tem uma entrada só: toda mensagem entra num projeto e produz desenho. Quem
...
// mesma do resto do Assistente: **a LLM classifica e descreve; o código decide e executa.**
...
export type AssistantMode = 'answer' | 'propose' | 'operate' | 'explain'
export const ASSISTANT_MODES: readonly AssistantMode[] = ['answer', 'propose', 'operate', 'explain']
 ...
 */
export function parseIntent(bruto: unknown, mensagemOriginal = ''): AssistantIntent {
  const fallback = (): AssistantIntent => ({ mode: 'answer', query: texto(mensagemOriginal), freshness: 'static' })
  if (!bruto || typeof bruto !== 'object') return fallback()

  const o = bruto as Record<string, unknown>
  const mode = String(o.mode ?? '') as AssistantMode
  if (!ASSISTANT_MODES.includes(mode)) return fallback()
 ...
 */
export function policyFor(intent: AssistantIntent): IntentPolicy {
  switch (intent.mode) {
    case 'answer':
      return {
 ...
 */
export function suggestIntent(mensagem: string): AssistantIntent {
 ...
 */
export function clarifyingQuestion(mensagem: string, intent: AssistantIntent): string | null {
  const m = String(mensagem ?? '').toLowerCase()
