...
import { ASSISTANT_CONSTITUTION_VERSION } from './constitution.js'
...
import type { AssistantProject } from './repository.js'
import { BLUEPRINT_LAYERS } from './types.js'
import type { AssistantAssumption, BlueprintLayer, OfficeBlueprintV1 } from './types.js'
...
const requireProject = async (ownerId: string, id: ObjectId): Promise<AssistantProject> => {
  const p = await repo.getProject(ownerId, id)
  // O de outra conta e o inexistente são o MESMO caso para quem chama.
  if (!p) throw new AssistantRefusal('no_blueprint', 'projeto não encontrado')
  return p
}
...
export async function createProject(ownerId: string, input: { objective: string; title?: string; provider?: 'anthropic' | 'openai'; model?: string | null }): Promise<AssistantProject> {
  ...
  if ((await repo.countProjects(ownerId)) >= L.MAX_PROJECTS_PER_OWNER) {
    throw new AssistantRefusal('too_many_projects', `você já tem ${L.MAX_PROJECTS_PER_OWNER} projetos; arquive um para começar outro`)
  }
...
export async function sendMessage(
...
): Promise<{ project: AssistantProject; assistantText: string; question: unknown; secretMasked: boolean }> {
  ...
  if (!isConversable(projeto.status)) throw new AssistantRefusal('not_editable', 'este projeto foi arquivado')
  ...
  if ((await repo.countMessages(ownerId, projectId)) >= L.MAX_MESSAGES_PER_PROJECT) {
    throw new AssistantRefusal('too_many_messages', 'esta conversa chegou ao limite; gere a proposta ou comece um projeto novo')
  }
  ...
  return runTurn(ownerId, projeto, `assistant:${projectId.toString()}:${mensagem._id.toString()}`, {
...
export async function advanceTurn(
  ...
  return runTurn(ownerId, projeto, `assistant:${projectId.toString()}:turn:${opts.forceProposal ? 'gen:' : ''}${marca}`, {
...
async function runTurn(
  ownerId: string,
  projeto: AssistantProject,
  chargeKey: string,
  opts: { forceProposal?: boolean; secretMasked: boolean; answeringPending?: string },
): Promise<{ project: AssistantProject; assistantText: string; question: unknown; secretMasked: boolean }> {
  // O escritório atual entra junto: sem ele o Assistente propõe criar o que já existe.
  const [messages, apps, existing, manifesto] = await Promise.all([
    ...
    // é o que impede o Assistente de propor o App que a pessoa acabou de desconectar.
  ...
  if (!resultado.ok) {
    ...
    throw new AssistantRefusal(resultado.failure.code, resultado.failure.message)
  }

  const turno = resultado.result
  // A mensagem do Assistente é gravada DEPOIS de o plano existir — ver `resumoDaMudanca`
  ...
  const patch: Partial<AssistantProject> = {
    ...
    assistantConstitutionVersion: ASSISTANT_CONSTITUTION_VERSION,
 ...
 * fica como está.
 */
async function marcarOQueJaExiste(ownerId: string, projeto: AssistantProject, base: OfficeBlueprintV1 | null): Promise<OfficeBlueprintV1 | null> {
  if (!base) return base
 ...
 */
async function marcarOQueJaExisteV2(ownerId: string, projeto: AssistantProject, base: OfficeBlueprintV2 | null): Promise<OfficeBlueprintV2 | null> {
  if (!base) return base
  const operacao = await repo.lastOperation(ownerId, projeto._id)
  const mapa = new Map(Object.entries(operacao?.resourceMap ?? {}))
  if (mapa.size === 0) return base
...
function mesclarSuposicoes(anteriores: AssistantAssumption[], novas: AssistantAssumption[], answers: Record<string, unknown>): AssistantAssumption[] {
  const porChave = new Map<string, AssistantAssumption>()
...
const marcadosDe = (projeto: AssistantProject): Set<string> =>
...
export async function previewProject(ownerId: string, projectId: ObjectId) {
  const projeto = await requireProject(ownerId, projectId)
  if (!projeto.blueprint) throw new AssistantRefusal('no_blueprint', 'ainda não existe proposta para revisar')
...
export async function patchProjectFields(
  ...
  const projeto = await requireProject(ownerId, projectId)
  if (!isEditable(projeto.status)) throw new AssistantRefusal('not_editable', 'este projeto já foi aplicado ou arquivado')
  const set: Partial<AssistantProject> = {}
...
export async function markChecklistItem(ownerId: string, projectId: ObjectId, itemId: string, done: boolean) {
  ...
  if (!item) throw new AssistantRefusal('no_blueprint', 'item não encontrado')
...
export async function archiveProject(ownerId: string, projectId: ObjectId): Promise<AssistantProject> {
  const projeto = await requireProject(ownerId, projectId)
  ...
  if (!atualizado) throw new AssistantRefusal('not_editable', 'não dá para arquivar um projeto que está sendo aplicado')
  return atualizado
}
...
export async function deleteProject(ownerId: string, projectId: ObjectId): Promise<{ id: string; title: string }> {
  const projeto = await requireProject(ownerId, projectId)
  if (projeto.status === 'applying') throw new AssistantRefusal('not_editable', 'não dá para apagar um projeto que está sendo aplicado')
...
export const projectSummary = (p: AssistantProject) => ({
  id: p._id.toString(),
  title: p.title,
...
export const projectDetail = (p: AssistantProject) => ({
  ...projectSummary(p),
  provider: p.provider,
  model: p.model,
  answers: p.answers,
  ...
  // que a pessoa corrige o Assistente sem precisar reabrir a conversa inteira.
  brief: p.brief ?? null,
...
export async function applyProject(
  ...
  if (projeto.status === 'applied') {
    ...
    throw new AssistantRefusal('not_editable', 'este projeto já foi aplicado')
  }

  const travado = await repo.transitionProject(ownerId, projectId, ['ready'], 'applying')
  if (!travado) {
    throw new AssistantRefusal('not_editable', projeto.status === 'applying' ? 'esta proposta já está sendo aplicada' : 'valide a proposta antes de aplicar')
  }
  ...
  try {
    ...
    if (error instanceof ApplyConflict) throw new AssistantRefusal('not_editable', error.message)
    throw error
  }
}

/** Liga itens da proposta a recursos reais desta conta. O id nunca vem do modelo. */
export async function setBlueprintLinks(ownerId: string, projectId: ObjectId, links: unknown): Promise<AssistantProject> {
  const projeto = await requireProject(ownerId, projectId)
  if (!isEditable(projeto.status)) throw new AssistantRefusal('not_editable', 'este projeto já foi aplicado ou arquivado')
  if (!projeto.blueprint) throw new AssistantRefusal('no_blueprint', 'ainda não existe proposta para ligar')
 ...
 */
export async function setProjectLayer(ownerId: string, projectId: ObjectId, layer: unknown): Promise<AssistantProject> {
  const projeto = await requireProject(ownerId, projectId)
  if (!isEditable(projeto.status)) throw new AssistantRefusal('not_editable', 'este projeto já foi aplicado ou arquivado')
  if (!projeto.blueprint) throw new AssistantRefusal('no_blueprint', 'ainda não existe proposta para recortar')
 ...
 */
export async function editBlueprint(ownerId: string, projectId: ObjectId, edits: unknown): Promise<AssistantProject> {
  const projeto = await requireProject(ownerId, projectId)
  if (!isEditable(projeto.status)) throw new AssistantRefusal('not_editable', 'este projeto já foi aplicado ou arquivado')
  if (!projeto.blueprint) throw new AssistantRefusal('no_blueprint', 'ainda não existe proposta para editar')
 ...
 */
export async function editBrief(ownerId: string, projectId: ObjectId, patch: unknown): Promise<AssistantProject> {
  const projeto = await requireProject(ownerId, projectId)
  if (!isConversable(projeto.status)) throw new AssistantRefusal('not_editable', 'este projeto foi arquivado')
...
export async function undoBrief(ownerId: string, projectId: ObjectId): Promise<AssistantProject> {
  const projeto = await requireProject(ownerId, projectId)
 ...
 */
function recompilar(projeto: AssistantProject, brief: OperationBrief, manifesto: AssistantCapabilityManifest | null): Partial<AssistantProject> {
...
export const assistantTargets = (ownerId: string) => loadTargets(ownerId)

/** Para onde ir depois de aplicar — recalculado do banco, não guardado na tela. */
export const projectLinks = (ownerId: string, projeto: AssistantProject) => appliedLinks(ownerId, projeto)

export async function resumeProject(ownerId: string, projectId: ObjectId, hooks: ApplyHooks = {}) {
  const projeto = await requireProject(ownerId, projectId)
  if (!RESUME_FROM.includes(projeto.status)) throw new AssistantRefusal('not_editable', 'não há aplicação interrompida para retomar')
...
export async function rollbackProject(ownerId: string, projectId: ObjectId) {
  ...
  if (!operacao) throw new AssistantRefusal('no_blueprint', 'não há aplicação para desfazer')
