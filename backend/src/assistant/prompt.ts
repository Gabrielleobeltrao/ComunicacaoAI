import { ASSISTANT_MARKER } from '../llmFake.js'
import { constitutionForPrompt } from './constitution.js'
import * as L from './limits.js'
import type { AssistantMessage, AssistantProject } from './repository.js'
...
export { ASSISTANT_MARKER }

const REGRAS = `Você é o Assistente do Escritório. Você PROPÕE uma operação; quem cria os recursos é o sistema, depois da confirmação da pessoa.
...
export function buildAssistantPrompt(input: {
  project: Pick<AssistantProject, 'title' | 'objective' | 'locale' | 'answers' | 'blueprint'>
  messages: Pick<AssistantMessage, 'role' | 'content'>[]
  apps: { key: string; name: string; connected: boolean }[]
  /** O que a conta JÁ tem. Sem isto o Assistente propõe duplicar o escritório. */
