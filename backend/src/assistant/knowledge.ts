...
// que ela sempre foi — fato acumulado por execução —, e o que o Assistente grava é
 ...
 */
export async function writeAssistantKnowledge(
  ownerId: string,
 ...
 */
export async function assistantKnowledgeExists(ownerId: string, _scope: KnowledgeScope, id: string): Promise<boolean> {
 ...
 */
export async function deleteAssistantKnowledge(_ownerId: string, _scope: KnowledgeScope, id: string): Promise<boolean> {
  if (!ObjectId.isValid(id)) return false
