...
function funcaoQueServe(job: BriefJob, manifest: AssistantCapabilityManifest | null): string | undefined {
...
function appQueServe(job: BriefJob, manifest: AssistantCapabilityManifest | null): string | undefined {
  if (!manifest) return undefined
  const alvo = texto(job)
...
export function classifyJob(job: BriefJob, manifest: AssistantCapabilityManifest | null, escolhida: ResourceKind | null = null): ResourceDecision {
  const alvo = texto(job)
  const temDecisao = Boolean(job.decision && job.decision.trim())
 ...
 */
function comEscolhaDaPessoa(decisao: ResourceDecision, escolhida: ResourceKind | null, job: BriefJob, manifest: AssistantCapabilityManifest | null): ResourceDecision {
