...
import type { AssistantChecklistItem, AssistantReadiness, OfficeBlueprintV1 } from './types.js'
...
export function deriveChecklist(bp: OfficeBlueprintV1): AssistantChecklistItem[] {
  const itens: AssistantChecklistItem[] = []
  ...
  for (const req of bp.knowledgeRequirements ?? []) {
    itens.push({
      ...
      /**
       * O CAMINHO de resolver. O Assistente não anexa arquivo — e não anexar é a decisão
...
export function computeReadiness(itens: AssistantChecklistItem[], blockers: string[] = []): AssistantReadiness {
  const obrigatorios = itens.filter((i) => i.required)
  const opcionais = itens.filter((i) => !i.required)
  const feitos = (lista: AssistantChecklistItem[]) => lista.filter((i) => i.status === 'done').length
